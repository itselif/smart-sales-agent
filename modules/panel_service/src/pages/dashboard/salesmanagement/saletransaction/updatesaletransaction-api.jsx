...
export default function SalesManagementUpdateSaleTransactionApiPage() {
  const [view, setView] = useState("Table");
  const [updatedSaletransaction, setUpdatedSaletransaction] = useState(null);
  ...
  const onSubmit = handleSubmit(async (data) => {
    try {
      ...
      setError(null);
      setUpdatedSaletransaction(null);
      reset();
      console.info("RESPONSE", response);
      setUpdatedSaletransaction(response.data.saleTransaction);
    } catch (ex) {
      console.error(ex);
      setError(ex);
    }
    ...
    <Form methods={methods} onSubmit={onSubmit}>
      ...
      {(updatedSaletransaction || error) && (
        <Box paddingTop="2rem">
          ...
          <Box>
            {view === "Table" ? (
              <TableResponse content={updatedSaletransaction} error={error} />
            ) : (
              <JsonResponse content={updatedSaletransaction || error} />
            )}
          </Box>
        </Box>
      )}
