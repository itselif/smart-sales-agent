...
const requestParams = [{ name: "status", value: "", type: "Enum" }];

const ReportrequestSchema = zod.object({
  status: zod.string().nullable(),
});

export default function ReportingUpdateReportRequestApiPage() {
  const [view, setView] = useState("Table");
  const [updatedReportrequest, setUpdatedReportrequest] = useState(null);
  ...
  const onSubmit = handleSubmit(async (data) => {
    try {
      const response = await reportingAxios.patch(
        reportingEndpoints.reportRequest.updateReportRequest.replace(
          ":reportRequestId",
      ...
      setError(null);
      setUpdatedReportrequest(null);
      reset();
      console.info("RESPONSE", response);
      setUpdatedReportrequest(response.data.reportRequest);
    } catch (ex) {
      console.error(ex);
      setError(ex);
    }
    ...
    <Form methods={methods} onSubmit={onSubmit}>
      ...
      {(updatedReportrequest || error) && (
        <Box paddingTop="2rem">
          ...
          <Box>
            {view === "Table" ? (
              <TableResponse content={updatedReportrequest} error={error} />
            ) : (
              <JsonResponse content={updatedReportrequest || error} />
            )}
          </Box>
        </Box>
      )}
