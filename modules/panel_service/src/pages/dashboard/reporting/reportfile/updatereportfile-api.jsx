...
export default function ReportingUpdateReportFileApiPage() {
  const [view, setView] = useState("Table");
  const [updatedReportfile, setUpdatedReportfile] = useState(null);
  ...
  const onSubmit = handleSubmit(async (data) => {
    try {
      ...
      setError(null);
      setUpdatedReportfile(null);
      reset();
      console.info("RESPONSE", response);
      setUpdatedReportfile(response.data.reportFile);
    } catch (ex) {
      console.error(ex);
      setError(ex);
    }
    ...
    <Form methods={methods} onSubmit={onSubmit}>
      ...
      {(updatedReportfile || error) && (
        <Box paddingTop="2rem">
          ...
          <Box>
            {view === "Table" ? (
              <TableResponse content={updatedReportfile} error={error} />
            ) : (
              <JsonResponse content={updatedReportfile || error} />
            )}
          </Box>
        </Box>
      )}
