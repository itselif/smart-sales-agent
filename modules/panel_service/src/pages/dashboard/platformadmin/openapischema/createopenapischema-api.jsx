...
export default function PlatformAdminCreateOpenApiSchemaApiPage() {
  const [view, setView] = useState("Table");
  const [createdOpenapischema, setCreatedOpenapischema] = useState(null);
  ...
  const onSubmit = handleSubmit(async (data) => {
    try {
      ...
      setError(null);
      setCreatedOpenapischema(null);
      reset();
      console.info("RESPONSE", response);
      setCreatedOpenapischema(response.data.openapischema);
    } catch (ex) {
      console.error(ex);
      setError(ex);
    }
    ...
    <Form methods={methods} onSubmit={onSubmit}>
      ...
      {(createdOpenapischema || error) && (
        <Box paddingTop="2rem">
          ...
          <Box>
            {view === "Table" ? (
              <TableResponse content={createdOpenapischema} error={error} />
            ) : (
              <JsonResponse content={createdOpenapischema || error} />
            )}
          </Box>
        </Box>
      )}
