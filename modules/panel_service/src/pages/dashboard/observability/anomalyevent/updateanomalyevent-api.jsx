...
export default function ObservabilityUpdateAnomalyEventApiPage() {
  const [view, setView] = useState("Table");
  const [updatedAnomalyevent, setUpdatedAnomalyevent] = useState(null);
  ...
  const onSubmit = handleSubmit(async (data) => {
    try {
      ...
      setError(null);
      setUpdatedAnomalyevent(null);
      reset();
      console.info("RESPONSE", response);
      setUpdatedAnomalyevent(response.data.anomalyEvent);
    } catch (ex) {
      console.error(ex);
      setError(ex);
    }
    ...
    <Form methods={methods} onSubmit={onSubmit}>
      ...
      {(updatedAnomalyevent || error) && (
        <Box paddingTop="2rem">
          ...
          <Box>
            {view === "Table" ? (
              <TableResponse content={updatedAnomalyevent} error={error} />
            ) : (
              <JsonResponse content={updatedAnomalyevent || error} />
            )}
          </Box>
        </Box>
      )}
