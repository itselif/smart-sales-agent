...
export default function InventoryManagementCreateLowStockAlertApiPage() {
  const [view, setView] = useState("Table");
  const [createdLowstockalert, setCreatedLowstockalert] = useState(null);
  ...
  const onSubmit = handleSubmit(async (data) => {
    try {
      ...
      setError(null);
      setCreatedLowstockalert(null);
      reset();
      console.info("RESPONSE", response);
      setCreatedLowstockalert(response.data.lowstockalert);
    } catch (ex) {
      console.error(ex);
      setError(ex);
    }
    ...
    <Form methods={methods} onSubmit={onSubmit}>
      ...
      {(createdLowstockalert || error) && (
        <Box paddingTop="2rem">
          ...
          <Box>
            {view === "Table" ? (
              <TableResponse content={createdLowstockalert} error={error} />
            ) : (
              <JsonResponse content={createdLowstockalert || error} />
            )}
          </Box>
        </Box>
      )}
