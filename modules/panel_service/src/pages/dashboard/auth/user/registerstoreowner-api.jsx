...
const StoreownerSchema = zod.object({
...
export default function AuthRegisterStoreOwnerApiPage() {
  const [view, setView] = useState("Table");
  const [createdStoreowner, setCreatedStoreowner] = useState(null);
  ...
  const onSubmit = handleSubmit(async (data) => {
    try {
      const response = await authAxios.post(
        authEndpoints.user.registerStoreOwner,
        data,
      );
      setError(null);
      setCreatedStoreowner(null);
      reset();
      console.info("RESPONSE", response);
      setCreatedStoreowner(response.data.storeowner);
    } catch (ex) {
      console.error(ex);
      setError(ex);
    }
    ...
    <Form methods={methods} onSubmit={onSubmit}>
      ...
      {(createdStoreowner || error) && (
        <Box paddingTop="2rem">
          ...
          <Box>
            {view === "Table" ? (
              <TableResponse content={createdStoreowner} error={error} />
            ) : (
              <JsonResponse content={createdStoreowner || error} />
            )}
          </Box>
        </Box>
      )}
