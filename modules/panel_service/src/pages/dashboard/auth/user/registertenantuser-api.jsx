...
export default function AuthRegisterTenantUserApiPage() {
  const [view, setView] = useState("Table");
  const [createdTenantuser, setCreatedTenantuser] = useState(null);
  ...
  const onSubmit = handleSubmit(async (data) => {
    try {
      const response = await authAxios.post(
        authEndpoints.user.registerTenantUser,
        data,
      );
      setError(null);
      setCreatedTenantuser(null);
      reset();
      console.info("RESPONSE", response);
      setCreatedTenantuser(response.data.tenantuser);
    } catch (ex) {
      console.error(ex);
      setError(ex);
    }
    ...
    <Form methods={methods} onSubmit={onSubmit}>
      ...
      {(createdTenantuser || error) && (
        <Box paddingTop="2rem">
          ...
          <Box>
            {view === "Table" ? (
              <TableResponse content={createdTenantuser} error={error} />
            ) : (
              <JsonResponse content={createdTenantuser || error} />
            )}
          </Box>
        </Box>
      )}
