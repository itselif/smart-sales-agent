...
export default function AuthCreateStoreApiPage() {
  ...
  const onSubmit = handleSubmit(async (data) => {
    try {
      const response = await authAxios.post(
        authEndpoints.store.createStore,
        data,
      );
      setError(null);
    ...
    <Form methods={methods} onSubmit={onSubmit}>
      ...
      {(createdStore || error) && (
        <Box paddingTop="2rem">
          ...
          <Box>
            {view === "Table" ? (
              <TableResponse content={createdStore} error={error} />
            ) : (
              <JsonResponse content={createdStore || error} />
            )}
          </Box>
        </Box>
      )}
