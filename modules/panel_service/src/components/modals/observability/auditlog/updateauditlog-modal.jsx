...
export default function ObservabilityUpdateAuditLogModal({
  openDialog,
  selectedAuditLog,
}) {
  ...
  const requestParams = [
    { name: "message", value: selectedAuditLog?.message ?? "", type: "Text" },
  ];

  const defaultValues = {
    message: selectedAuditLog?.message ?? "",
  };
    ...
    try {
      if (selectedAuditLog?.id) {
        const response = await observabilityAxios.patch(
          observabilityEndpoints.auditLog.updateAuditLog.replace(
            ":auditLogId",
            selectedAuditLog?.id,
          ),
          data,
