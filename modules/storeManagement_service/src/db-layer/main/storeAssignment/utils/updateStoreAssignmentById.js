...
const indexDataToElastic = async (data) => {
  const elasticIndexer = new ElasticIndexer("storeAssignment");
  await elasticIndexer.indexData(data);
};

const updateStoreAssignmentById = async (id, dataClause) => {
  try {
    if (!id && dataClause.id) {
      id = dataClause.id;
    ...
    const existingDoc = await StoreAssignment.findOne({
    ...
    const [rowsCount, [dbDoc]] = await StoreAssignment.update(
      dataClause,
      options,
    );
    if (!dbDoc) {
...
module.exports = updateStoreAssignmentById;
