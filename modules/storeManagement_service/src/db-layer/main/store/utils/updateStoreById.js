...
const updateStoreById = async (id, dataClause) => {
  try {
    if (!id && dataClause.id) {
      id = dataClause.id;
    ...
    const existingDoc = await Store.findOne({ where: { id, isActive: true } });
    ...
    const [rowsCount, [dbDoc]] = await Store.update(dataClause, options);
    if (!dbDoc) {
...
module.exports = updateStoreById;
