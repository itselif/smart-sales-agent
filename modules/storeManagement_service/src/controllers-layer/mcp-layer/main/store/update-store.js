...
class UpdateStoreMcpController extends StoreManagementMcpController {
  constructor(params) {
    super("updateStore", "updatestore", params);
    this.dataName = "store";
    this.crudType = "update";
  }

  createApiManager() {
    return new UpdateStoreManager(this.request, "mcp");
  }
...
module.exports = (headers) => {
  return {
    ...
    parameters: UpdateStoreMcpController.getInputScheme(),
    controller: async (mcpParams) => {
      mcpParams.headers = headers;
      const updateStoreMcpController = new UpdateStoreMcpController(mcpParams);
      try {
        const result = await updateStoreMcpController.processRequest();
        //return UpdateStoreMcpController.getOutputSchema().parse(result);
        return {
          content: [
            {
              type: "text",
