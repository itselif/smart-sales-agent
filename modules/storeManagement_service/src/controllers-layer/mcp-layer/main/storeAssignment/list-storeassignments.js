...
class ListStoreAssignmentsMcpController extends StoreManagementMcpController {
  constructor(params) {
    super("listStoreAssignments", "liststoreassignments", params);
    this.dataName = "storeAssignments";
    this.crudType = "getList";
  }

  createApiManager() {
    return new ListStoreAssignmentsManager(this.request, "mcp");
  }
...
module.exports = (headers) => {
  return {
    ...
    description:
      "List all assignments (with filter, paging; e.g. by user, store, status, type).",
    parameters: ListStoreAssignmentsMcpController.getInputScheme(),
    controller: async (mcpParams) => {
      mcpParams.headers = headers;
      const listStoreAssignmentsMcpController =
        new ListStoreAssignmentsMcpController(mcpParams);
      try {
        const result = await listStoreAssignmentsMcpController.processRequest();
        //return ListStoreAssignmentsMcpController.getOutputSchema().parse(result);
        return {
          content: [
            {
              type: "text",
