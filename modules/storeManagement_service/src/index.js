...
console.log("Starting salesai-storemanagement-service service...");
...
const start = async () => {
  process.title = "node-salesai1-storemanagement-service";
  ...
  try {
    ...
    await setRedisData(
      "salesai1-storemanagement-service:start",
      today.toUTCString(),
    );
    console.log(
      "Service start datetime is written to Redis key salesai1-storemanagement-service:start",
      today.toUTCString(),
    );
  } catch (err) {
  ...
  console.log(
    "salesai-storemanagement-service is listening HTTP/REST port " +
...
/* starting the server salesai-storemanagement-service .... */
start();
