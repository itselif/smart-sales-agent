...
describe("updateStoreAssignmentByQuery module", () => {
  let sandbox;
  let updateStoreAssignmentByQuery;
  ...
  beforeEach(() => {
    ...
    updateStoreAssignmentByQuery = proxyquire(
      "../../../../../src/db-layer/main/StoreAssignment/utils/updateStoreAssignmentByQuery",
      {
  ...
  describe("updateStoreAssignmentByQuery", () => {
    it("should update records matching query and return getData list", async () => {
      const result = await updateStoreAssignmentByQuery(
        fakeDataClause,
        fakeQuery,
      );
      ...
      const [calledDataClause, calledOptions] =
        StoreAssignmentStub.update.firstCall.args;
    ...
    it("should return list of undefineds if getData() returns undefined", async () => {
      StoreAssignmentStub.update.resolves([
        2,
    ...
    it("should throw BadRequestError if query is undefined", async () => {
      try {
        await updateStoreAssignmentByQuery(fakeDataClause, undefined);
    ...
    it("should throw BadRequestError if query is not an object", async () => {
      try {
        await updateStoreAssignmentByQuery(fakeDataClause, "not-an-object");
        throw new Error("Expected error");
      } catch (err) {
        expect(err.name).to.equal("HttpServerError");
        expect(err.details.name).to.equal("BadRequestError");
    ...
    it("should throw HttpServerError if model update fails", async () => {
      ...
      try {
        await updateStoreAssignmentByQuery(fakeDataClause, fakeQuery);
        throw new Error("Expected error");
      } catch (err) {
        expect(err.name).to.equal("HttpServerError");
        expect(err.message).to.equal(
          "errMsg_dbErrorWhenUpdatingStoreAssignmentByQuery",
        );
    ...
    it("should accept empty dataClause and still process", async () => {
      ...
      const result = await updateStoreAssignmentByQuery({}, fakeQuery);
      expect(result).to.deep.equal([]);
    });
  });
});
