...
describe("deleteStoreAssignmentById module", () => {
  let sandbox;
  let deleteStoreAssignmentById;
  let StoreAssignmentStub, ElasticIndexerStub;
  let mockDocInstance;
  ...
  beforeEach(() => {
    ...
    mockDocInstance = {
      ...
      getData: () => ({ id: fakeId, name: "Deleted StoreAssignment" }),
    };
    ...
    deleteStoreAssignmentById = proxyquire(
      "../../../../../src/db-layer/main/StoreAssignment/utils/deleteStoreAssignmentById",
      {
  ...
  describe("deleteStoreAssignmentById", () => {
    it("should delete StoreAssignment and index removal in Elastic", async () => {
      const result = await deleteStoreAssignmentById(fakeId);

      expect(result).to.deep.equal({
        id: fakeId,
        name: "Deleted StoreAssignment",
      });
    ...
    it("should support object as ID input", async () => {
      ...
      StoreAssignmentStub.findOne.resolves(localMock);

      const result = await deleteStoreAssignmentById({ id: fakeId });
    ...
    it("should throw BadRequestError if no id is provided", async () => {
      try {
        await deleteStoreAssignmentById(undefined);
    ...
    it("should throw NotFoundError if record is not found", async () => {
      ...
      try {
        await deleteStoreAssignmentById(fakeId);
    ...
    it("should wrap internal errors with HttpServerError", async () => {
      StoreAssignmentStub.findOne.rejects(new Error("unexpected error"));
    ...
    it("should wrap ElasticIndexer.deleteData() error with HttpServerError", async () => {
      ...
      const result = await deleteStoreAssignmentById(fakeId).catch(
        (err) => err,
      );
    ...
    it("should accept numeric ID as valid input", async () => {
      ...
      const localMock = {
        id: numericId,
        update: sandbox.stub().resolves(),
        getData: () => ({ id: numericId, name: "Deleted StoreAssignment" }),
      };
      StoreAssignmentStub.findOne.resolves(localMock);

      const result = await deleteStoreAssignmentById(numericId);
      expect(result).to.deep.equal({
        id: numericId,
        name: "Deleted StoreAssignment",
      });
