...
describe("updateStoreAssignmentById module", () => {
  let sandbox;
  let updateStoreAssignmentById;
  let StoreAssignmentStub, ElasticIndexerStub;

  const fakeId = "uuid-123";
  const fakeUpdatedData = { id: fakeId, name: "Updated StoreAssignment" };
  ...
  beforeEach(() => {
    ...
    updateStoreAssignmentById = proxyquire(
      "../../../../../src/db-layer/main/StoreAssignment/utils/updateStoreAssignmentById",
      {
  ...
  describe("updateStoreAssignmentById", () => {
    it("should update record by direct ID", async () => {
      const result = await updateStoreAssignmentById(fakeId, {
        name: "Updated",
      });
    ...
    it("should extract ID from dataClause if not explicitly provided", async () => {
      const result = await updateStoreAssignmentById(undefined, {
        id: fakeId,
        name: "Updated",
      });
      expect(result).to.deep.equal(fakeUpdatedData);
    ...
    it("should support object-as-ID input", async () => {
      const result = await updateStoreAssignmentById(
        { id: fakeId },
        { name: "Updated" },
      );
      expect(result).to.deep.equal(fakeUpdatedData);
    ...
    it("should throw BadRequestError if no ID is provided", async () => {
      try {
        await updateStoreAssignmentById(undefined, {});
    ...
    it("should throw NotFoundError if no existing record is found", async () => {
      ...
      try {
        await updateStoreAssignmentById(fakeId, { name: "Updated" });
        throw new Error("Expected error");
      } catch (err) {
        expect(err.name).to.equal("HttpServerError");
    ...
    it("should throw HttpServerError if findOne fails", async () => {
      StoreAssignmentStub.findOne.rejects(new Error("DB fail"));
      try {
        await updateStoreAssignmentById(fakeId, { name: "Updated" });
        throw new Error("Expected error");
      } catch (err) {
