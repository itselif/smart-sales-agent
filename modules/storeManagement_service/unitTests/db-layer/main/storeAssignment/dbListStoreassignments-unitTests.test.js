...
//these tests will work when DbListStoreassignmentsCommand is exported from main code

describe("DbListStoreassignmentsCommand", () => {
  let DbListStoreassignmentsCommand, dbListStoreassignments;
  ...
  beforeEach(() => {
    ...
    ({ DbListStoreassignmentsCommand, dbListStoreassignments } = proxyquire(
      "../../../../src/db-layer/main/storeAssignment/dbListStoreassignments",
      {
  ...
  describe("constructor", () => {
    it("should initialize command metadata and reset pagination if needed", () => {
      ...
      const cmd = new DbListStoreassignmentsCommand(input);

      expect(cmd.commandName).to.equal("dbListStoreassignments");
      expect(cmd.emptyResult).to.be.true;
      expect(cmd.objectName).to.equal("storeAssignments");
  ...
  describe("getCqrsJoins", () => {
    it("should call StoreAssignment.getCqrsJoins if defined", async () => {
      const cmd = new DbListStoreassignmentsCommand({});
  ...
  describe("buildIncludes", () => {
    ...
    it("should return [] includes even if getJoins is absent", () => {
      const cmd = new DbListStoreassignmentsCommand({}); // input.getJoins is undefined
  ...
  describe("executeQuery", () => {
    it("should handle empty selectList and skip attributes", async () => {
      const cmd = new DbListStoreassignmentsCommand({ getJoins: false });
      sandbox.stub(cmd, "getSelectList").returns(null);

      await cmd.executeQuery();

      const callArgs = StoreAssignmentStub.findAll.getCall(0).args[0];
    ...
    it("should return rows with count if pagination and count query are triggered", async () => {
      StoreAssignmentStub.findAll.reset();
      StoreAssignmentStub.findAll.onCall(0).resolves([{ id: 1 }, { id: 2 }]);
      StoreAssignmentStub.findAll.onCall(1).resolves([{ _COUNT: 12 }]);
      ...
      cmd.executeQuery = async function () {
        const data = await StoreAssignmentStub.findAll();
        const count = await StoreAssignmentStub.findAll();
        return {
          rows: data,
    ...
    it("should convert aggregations to float or int if convertAggregationsToNumbers exists", async () => {
      ...
      StoreAssignmentStub.findAll.reset();
      StoreAssignmentStub.findAll.resolves(data);
    ...
    it("should return first element directly if isAggregatedWithoutGroup", async () => {
      ...
      sandbox.stub(cmd, "getSelectList").returns(["total"]);
      sandbox.stub(cmd, "buildIncludes").returns(null);
      StoreAssignmentStub.findAll.resolves([first]);
