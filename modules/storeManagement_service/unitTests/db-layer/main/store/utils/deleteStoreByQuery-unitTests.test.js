...
describe("deleteStoreByQuery module", () => {
  let sandbox;
  let deleteStoreByQuery;
  let StoreStub;
  ...
  beforeEach(() => {
    ...
    deleteStoreByQuery = proxyquire(
      "../../../../../src/db-layer/main/Store/utils/deleteStoreByQuery",
      {
  ...
  describe("deleteStoreByQuery", () => {
    ...
    it("should throw BadRequestError if query is undefined", async () => {
      try {
        await deleteStoreByQuery(undefined);
    ...
    it("should throw BadRequestError if query is not an object", async () => {
      try {
        await deleteStoreByQuery("string");
        throw new Error("Expected BadRequestError");
      } catch (err) {
        expect(err.name).to.equal("HttpServerError");
        expect(err.details.name).to.equal("BadRequestError");
    ...
    it("should wrap model update() error in HttpServerError", async () => {
      StoreStub.update.rejects(new Error("update error"));

      try {
        await deleteStoreByQuery({ test: true });
        throw new Error("Expected HttpServerError");
      } catch (err) {
        expect(err.name).to.equal("HttpServerError");
        expect(err.message).to.equal("errMsg_dbErrorWhenDeletingStoreByQuery");
    ...
    it("should still return mapped array even if getData returns undefined", async () => {
      ...
      StoreStub.update.resolves([2, partial]);

      const result = await deleteStoreByQuery({ any: "field" });
