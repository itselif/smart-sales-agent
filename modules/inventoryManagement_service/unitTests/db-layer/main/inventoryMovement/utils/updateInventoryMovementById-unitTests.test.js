...
describe("updateInventoryMovementById module", () => {
  let sandbox;
  let updateInventoryMovementById;
  let InventoryMovementStub, ElasticIndexerStub;

  const fakeId = "uuid-123";
  const fakeUpdatedData = { id: fakeId, name: "Updated InventoryMovement" };
  ...
  beforeEach(() => {
    ...
    updateInventoryMovementById = proxyquire(
      "../../../../../src/db-layer/main/InventoryMovement/utils/updateInventoryMovementById",
      {
  ...
  describe("updateInventoryMovementById", () => {
    it("should update record by direct ID", async () => {
      const result = await updateInventoryMovementById(fakeId, {
        name: "Updated",
      });
    ...
    it("should extract ID from dataClause if not explicitly provided", async () => {
      const result = await updateInventoryMovementById(undefined, {
        id: fakeId,
        name: "Updated",
      });
      expect(result).to.deep.equal(fakeUpdatedData);
    ...
    it("should support object-as-ID input", async () => {
      const result = await updateInventoryMovementById(
        { id: fakeId },
        { name: "Updated" },
      );
      expect(result).to.deep.equal(fakeUpdatedData);
    ...
    it("should throw BadRequestError if no ID is provided", async () => {
      try {
        await updateInventoryMovementById(undefined, {});
    ...
    it("should throw NotFoundError if no existing record is found", async () => {
      ...
      try {
        await updateInventoryMovementById(fakeId, { name: "Updated" });
        throw new Error("Expected error");
      } catch (err) {
        expect(err.name).to.equal("HttpServerError");
    ...
    it("should throw HttpServerError if findOne fails", async () => {
      InventoryMovementStub.findOne.rejects(new Error("DB fail"));
      try {
        await updateInventoryMovementById(fakeId, { name: "Updated" });
        throw new Error("Expected error");
      } catch (err) {
