...
describe("updateInventoryMovementByIdList module", () => {
  let sandbox;
  let updateInventoryMovementByIdList;
  ...
  beforeEach(() => {
    ...
    updateInventoryMovementByIdList = proxyquire(
      "../../../../../src/db-layer/main/InventoryMovement/utils/updateInventoryMovementByIdList",
      {
  ...
  describe("updateInventoryMovementByIdList", () => {
    it("should return list of updated IDs if update is successful", async () => {
      const result = await updateInventoryMovementByIdList(fakeIdList, {
        name: "Updated",
      });

      expect(result).to.deep.equal(["id1", "id2"]);
      sinon.assert.calledOnce(InventoryMovementStub.update);
      const args = InventoryMovementStub.update.getCall(0).args;
    ...
    it("should return empty list if update returns no rows", async () => {
      ...
      const result = await updateInventoryMovementByIdList(["id99"], {
        status: "inactive",
      });
    ...
    it("should return list with one id if only one record updated", async () => {
      InventoryMovementStub.update.resolves([1, [{ id: "id1" }]]);

      const result = await updateInventoryMovementByIdList(["id1"], {
        active: false,
      });
    ...
    it("should throw HttpServerError if model update fails", async () => {
      ...
      try {
        await updateInventoryMovementByIdList(["id1"], { name: "err" });
    ...
    it("should call update with empty dataClause", async () => {
      await updateInventoryMovementByIdList(["id1"], {});
      sinon.assert.calledOnce(InventoryMovementStub.update);
    });
  });
});
