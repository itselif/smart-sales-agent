...
describe("deleteInventoryMovementById module", () => {
  let sandbox;
  let deleteInventoryMovementById;
  let InventoryMovementStub, ElasticIndexerStub;
  let mockDocInstance;
  ...
  beforeEach(() => {
    ...
    mockDocInstance = {
      ...
      getData: () => ({ id: fakeId, name: "Deleted InventoryMovement" }),
    };
    ...
    deleteInventoryMovementById = proxyquire(
      "../../../../../src/db-layer/main/InventoryMovement/utils/deleteInventoryMovementById",
      {
  ...
  describe("deleteInventoryMovementById", () => {
    it("should delete InventoryMovement and index removal in Elastic", async () => {
      const result = await deleteInventoryMovementById(fakeId);

      expect(result).to.deep.equal({
        id: fakeId,
        name: "Deleted InventoryMovement",
      });
    ...
    it("should support object as ID input", async () => {
      ...
      InventoryMovementStub.findOne.resolves(localMock);

      const result = await deleteInventoryMovementById({ id: fakeId });
    ...
    it("should throw BadRequestError if no id is provided", async () => {
      try {
        await deleteInventoryMovementById(undefined);
    ...
    it("should throw NotFoundError if record is not found", async () => {
      ...
      try {
        await deleteInventoryMovementById(fakeId);
    ...
    it("should wrap internal errors with HttpServerError", async () => {
      InventoryMovementStub.findOne.rejects(new Error("unexpected error"));
    ...
    it("should wrap ElasticIndexer.deleteData() error with HttpServerError", async () => {
      ...
      const result = await deleteInventoryMovementById(fakeId).catch(
        (err) => err,
      );
    ...
    it("should accept numeric ID as valid input", async () => {
      ...
      const localMock = {
        id: numericId,
        update: sandbox.stub().resolves(),
        getData: () => ({ id: numericId, name: "Deleted InventoryMovement" }),
      };
      InventoryMovementStub.findOne.resolves(localMock);

      const result = await deleteInventoryMovementById(numericId);
      expect(result).to.deep.equal({
        id: numericId,
        name: "Deleted InventoryMovement",
      });
