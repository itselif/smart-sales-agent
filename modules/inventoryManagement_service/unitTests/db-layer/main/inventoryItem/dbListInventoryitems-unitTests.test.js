...
//these tests will work when DbListInventoryitemsCommand is exported from main code

describe("DbListInventoryitemsCommand", () => {
  let DbListInventoryitemsCommand, dbListInventoryitems;
  ...
  beforeEach(() => {
    ...
    ({ DbListInventoryitemsCommand, dbListInventoryitems } = proxyquire(
      "../../../../src/db-layer/main/inventoryItem/dbListInventoryitems",
      {
  ...
  describe("constructor", () => {
    it("should initialize command metadata and reset pagination if needed", () => {
      ...
      const cmd = new DbListInventoryitemsCommand(input);

      expect(cmd.commandName).to.equal("dbListInventoryitems");
      expect(cmd.emptyResult).to.be.true;
      expect(cmd.objectName).to.equal("inventoryItems");
  ...
  describe("getCqrsJoins", () => {
    it("should call InventoryItem.getCqrsJoins if defined", async () => {
      const cmd = new DbListInventoryitemsCommand({});
  ...
  describe("buildIncludes", () => {
    ...
    it("should return [] includes even if getJoins is absent", () => {
      const cmd = new DbListInventoryitemsCommand({}); // input.getJoins is undefined
  ...
  describe("executeQuery", () => {
    it("should handle empty selectList and skip attributes", async () => {
      const cmd = new DbListInventoryitemsCommand({ getJoins: false });
      sandbox.stub(cmd, "getSelectList").returns(null);

      await cmd.executeQuery();

      const callArgs = InventoryItemStub.findAll.getCall(0).args[0];
    ...
    it("should return rows with count if pagination and count query are triggered", async () => {
      InventoryItemStub.findAll.reset();
      InventoryItemStub.findAll.onCall(0).resolves([{ id: 1 }, { id: 2 }]);
      InventoryItemStub.findAll.onCall(1).resolves([{ _COUNT: 12 }]);
      ...
      cmd.executeQuery = async function () {
        const data = await InventoryItemStub.findAll();
        const count = await InventoryItemStub.findAll();
        return {
          rows: data,
    ...
    it("should convert aggregations to float or int if convertAggregationsToNumbers exists", async () => {
      ...
      InventoryItemStub.findAll.reset();
      InventoryItemStub.findAll.resolves(data);
    ...
    it("should return first element directly if isAggregatedWithoutGroup", async () => {
      ...
      sandbox.stub(cmd, "getSelectList").returns(["total"]);
      sandbox.stub(cmd, "buildIncludes").returns(null);
      InventoryItemStub.findAll.resolves([first]);
