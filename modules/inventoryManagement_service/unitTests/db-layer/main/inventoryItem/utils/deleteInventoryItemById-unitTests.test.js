...
describe("deleteInventoryItemById module", () => {
  let sandbox;
  let deleteInventoryItemById;
  let InventoryItemStub, ElasticIndexerStub;
  let mockDocInstance;
  ...
  beforeEach(() => {
    ...
    mockDocInstance = {
      ...
      getData: () => ({ id: fakeId, name: "Deleted InventoryItem" }),
    };
    ...
    deleteInventoryItemById = proxyquire(
      "../../../../../src/db-layer/main/InventoryItem/utils/deleteInventoryItemById",
      {
  ...
  describe("deleteInventoryItemById", () => {
    it("should delete InventoryItem and index removal in Elastic", async () => {
      const result = await deleteInventoryItemById(fakeId);

      expect(result).to.deep.equal({
        id: fakeId,
        name: "Deleted InventoryItem",
      });
    ...
    it("should support object as ID input", async () => {
      ...
      InventoryItemStub.findOne.resolves(localMock);

      const result = await deleteInventoryItemById({ id: fakeId });
    ...
    it("should throw BadRequestError if no id is provided", async () => {
      try {
        await deleteInventoryItemById(undefined);
    ...
    it("should throw NotFoundError if record is not found", async () => {
      ...
      try {
        await deleteInventoryItemById(fakeId);
    ...
    it("should wrap internal errors with HttpServerError", async () => {
      InventoryItemStub.findOne.rejects(new Error("unexpected error"));
    ...
    it("should wrap ElasticIndexer.deleteData() error with HttpServerError", async () => {
      ...
      const result = await deleteInventoryItemById(fakeId).catch((err) => err);
    ...
    it("should accept numeric ID as valid input", async () => {
      ...
      const localMock = {
        id: numericId,
        update: sandbox.stub().resolves(),
        getData: () => ({ id: numericId, name: "Deleted InventoryItem" }),
      };
      InventoryItemStub.findOne.resolves(localMock);

      const result = await deleteInventoryItemById(numericId);
      expect(result).to.deep.equal({
        id: numericId,
        name: "Deleted InventoryItem",
      });
