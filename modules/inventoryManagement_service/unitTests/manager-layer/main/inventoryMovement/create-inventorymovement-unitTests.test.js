...
describe("CreateInventoryMovement", () => {
  ...
  beforeEach(() => {
    ...
    ControllerClass = proxyquire(
      "../../../../src/manager-layer/main/InventoryMovement/create-inventorymovement",
      {
  ...
  it("should initialize controller with correct base properties", () => {
    ...
    expect(instance.options.name).to.equal("createInventoryMovement");
    expect(instance.options.controllerType).to.equal("rest");
    expect(instance.options.crudType).to.equal("create");
    expect(instance.dataName).to.equal("inventoryMovement");
  });
