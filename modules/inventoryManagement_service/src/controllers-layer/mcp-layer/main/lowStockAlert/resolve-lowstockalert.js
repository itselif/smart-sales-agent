...
class ResolveLowStockAlertMcpController extends InventoryManagementMcpController {
  constructor(params) {
    super("resolveLowStockAlert", "resolvelowstockalert", params);
    this.dataName = "lowStockAlert";
    this.crudType = "update";
  }

  createApiManager() {
    return new ResolveLowStockAlertManager(this.request, "mcp");
  }
  ...
  static getInputScheme() {
    return {
      ...
      resolvedByUserId: z
        ...
        .uuid()
        .optional()
        .describe("User who resolved or acknowledged the alert (optional)."),
...
module.exports = (headers) => {
  return {
    ...
    description:
      "Update alert status (e.g., set resolved or assign acknowledger/timestamp).",
    parameters: ResolveLowStockAlertMcpController.getInputScheme(),
    controller: async (mcpParams) => {
      mcpParams.headers = headers;
      const resolveLowStockAlertMcpController =
        new ResolveLowStockAlertMcpController(mcpParams);
      try {
        const result = await resolveLowStockAlertMcpController.processRequest();
        //return ResolveLowStockAlertMcpController.getOutputSchema().parse(result);
        return {
          content: [
            {
              type: "text",
