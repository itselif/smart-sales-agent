...
class GetLowStockAlertMcpController extends InventoryManagementMcpController {
  constructor(params) {
    super("getLowStockAlert", "getlowstockalert", params);
    this.dataName = "lowStockAlert";
    this.crudType = "get";
  }

  createApiManager() {
    return new GetLowStockAlertManager(this.request, "mcp");
  }
...
module.exports = (headers) => {
  return {
    ...
    parameters: GetLowStockAlertMcpController.getInputScheme(),
    controller: async (mcpParams) => {
      mcpParams.headers = headers;
      const getLowStockAlertMcpController = new GetLowStockAlertMcpController(
        mcpParams,
      );
      try {
        const result = await getLowStockAlertMcpController.processRequest();
        //return GetLowStockAlertMcpController.getOutputSchema().parse(result);
        return {
          content: [
            {
              type: "text",
