...
class CreateLowStockAlertMcpController extends InventoryManagementMcpController {
  constructor(params) {
    super("createLowStockAlert", "createlowstockalert", params);
    this.dataName = "lowStockAlert";
    this.crudType = "create";
  }

  createApiManager() {
    return new CreateLowStockAlertManager(this.request, "mcp");
  }
...
module.exports = (headers) => {
  return {
    ...
    description:
      "Create a low-stock alert event for an inventory item (usually system-triggered).",
    parameters: CreateLowStockAlertMcpController.getInputScheme(),
    controller: async (mcpParams) => {
      mcpParams.headers = headers;
      const createLowStockAlertMcpController =
        new CreateLowStockAlertMcpController(mcpParams);
      try {
        const result = await createLowStockAlertMcpController.processRequest();
        //return CreateLowStockAlertMcpController.getOutputSchema().parse(result);
        return {
          content: [
            {
              type: "text",
