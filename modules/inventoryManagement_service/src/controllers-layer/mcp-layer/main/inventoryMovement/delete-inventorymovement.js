...
class DeleteInventoryMovementMcpController extends InventoryManagementMcpController {
  constructor(params) {
    super("deleteInventoryMovement", "deleteinventorymovement", params);
    this.dataName = "inventoryMovement";
    this.crudType = "delete";
  }

  createApiManager() {
    return new DeleteInventoryMovementManager(this.request, "mcp");
  }
...
module.exports = (headers) => {
  return {
    ...
    description:
      "Soft-delete a movement record (if needed for audit correction).",
    parameters: DeleteInventoryMovementMcpController.getInputScheme(),
    controller: async (mcpParams) => {
      mcpParams.headers = headers;
      const deleteInventoryMovementMcpController =
        new DeleteInventoryMovementMcpController(mcpParams);
      try {
        const result =
          await deleteInventoryMovementMcpController.processRequest();
        //return DeleteInventoryMovementMcpController.getOutputSchema().parse(result);
        return {
          content: [
            {
              type: "text",
