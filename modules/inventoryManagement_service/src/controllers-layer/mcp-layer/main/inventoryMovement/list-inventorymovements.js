...
class ListInventoryMovementsMcpController extends InventoryManagementMcpController {
  constructor(params) {
    super("listInventoryMovements", "listinventorymovements", params);
    this.dataName = "inventoryMovements";
    this.crudType = "getList";
  }

  createApiManager() {
    return new ListInventoryMovementsManager(this.request, "mcp");
  }
...
module.exports = (headers) => {
  return {
    ...
    description:
      "Get a list/history of inventory movements for reporting or audit (filtered by item/store/user/timestamp/type).",
    parameters: ListInventoryMovementsMcpController.getInputScheme(),
    controller: async (mcpParams) => {
      mcpParams.headers = headers;
      const listInventoryMovementsMcpController =
        new ListInventoryMovementsMcpController(mcpParams);
      try {
        const result =
          await listInventoryMovementsMcpController.processRequest();
        //return ListInventoryMovementsMcpController.getOutputSchema().parse(result);
        return {
          content: [
            {
              type: "text",
