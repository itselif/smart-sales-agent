...
const indexDataToElastic = async (data) => {
  const elasticIndexer = new ElasticIndexer("inventoryMovement");
  await elasticIndexer.indexData(data);
};

const updateInventoryMovementById = async (id, dataClause) => {
  try {
    if (!id && dataClause.id) {
      id = dataClause.id;
    ...
    const existingDoc = await InventoryMovement.findOne({
    ...
    const [rowsCount, [dbDoc]] = await InventoryMovement.update(
      dataClause,
      options,
    );
    if (!dbDoc) {
...
module.exports = updateInventoryMovementById;
