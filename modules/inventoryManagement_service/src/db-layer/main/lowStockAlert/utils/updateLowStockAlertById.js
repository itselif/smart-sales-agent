...
const indexDataToElastic = async (data) => {
  const elasticIndexer = new ElasticIndexer("lowStockAlert");
  await elasticIndexer.indexData(data);
};

const updateLowStockAlertById = async (id, dataClause) => {
  try {
    if (!id && dataClause.id) {
      id = dataClause.id;
    ...
    const existingDoc = await LowStockAlert.findOne({
    ...
    const [rowsCount, [dbDoc]] = await LowStockAlert.update(
      dataClause,
      options,
    );
    if (!dbDoc) {
...
module.exports = updateLowStockAlertById;
