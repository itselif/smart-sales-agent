...
const indexDataToElastic = async (data) => {
  const elasticIndexer = new ElasticIndexer("inventoryItem");
  await elasticIndexer.indexData(data);
};

const updateInventoryItemById = async (id, dataClause) => {
  try {
    if (!id && dataClause.id) {
      id = dataClause.id;
    ...
    const existingDoc = await InventoryItem.findOne({
    ...
    const [rowsCount, [dbDoc]] = await InventoryItem.update(
      dataClause,
      options,
    );
    if (!dbDoc) {
...
module.exports = updateInventoryItemById;
