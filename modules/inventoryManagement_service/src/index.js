...
console.log("Starting salesai-inventorymanagement-service service...");
...
const start = async () => {
  process.title = "node-salesai1-inventorymanagement-service";
  ...
  try {
    ...
    await setRedisData(
      "salesai1-inventorymanagement-service:start",
      today.toUTCString(),
    );
    console.log(
      "Service start datetime is written to Redis key salesai1-inventorymanagement-service:start",
      today.toUTCString(),
    );
  } catch (err) {
  ...
  console.log(
    "salesai-inventorymanagement-service is listening HTTP/REST port " +
...
/* starting the server salesai-inventorymanagement-service .... */
start();
