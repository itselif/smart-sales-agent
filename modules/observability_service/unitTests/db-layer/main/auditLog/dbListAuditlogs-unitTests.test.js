...
//these tests will work when DbListAuditlogsCommand is exported from main code

describe("DbListAuditlogsCommand", () => {
  let DbListAuditlogsCommand, dbListAuditlogs;
  ...
  beforeEach(() => {
    ...
    ({ DbListAuditlogsCommand, dbListAuditlogs } = proxyquire(
      "../../../../src/db-layer/main/auditLog/dbListAuditlogs",
      {
  ...
  describe("constructor", () => {
    it("should initialize command metadata and reset pagination if needed", () => {
      ...
      const cmd = new DbListAuditlogsCommand(input);

      expect(cmd.commandName).to.equal("dbListAuditlogs");
      expect(cmd.emptyResult).to.be.true;
      expect(cmd.objectName).to.equal("auditLogs");
  ...
  describe("getCqrsJoins", () => {
    it("should call AuditLog.getCqrsJoins if defined", async () => {
      const cmd = new DbListAuditlogsCommand({});
  ...
  describe("buildIncludes", () => {
    ...
    it("should return [] includes even if getJoins is absent", () => {
      const cmd = new DbListAuditlogsCommand({}); // input.getJoins is undefined
  ...
  describe("executeQuery", () => {
    it("should handle empty selectList and skip attributes", async () => {
      const cmd = new DbListAuditlogsCommand({ getJoins: false });
      sandbox.stub(cmd, "getSelectList").returns(null);

      await cmd.executeQuery();

      const callArgs = AuditLogStub.findAll.getCall(0).args[0];
    ...
    it("should return rows with count if pagination and count query are triggered", async () => {
      AuditLogStub.findAll.reset();
      AuditLogStub.findAll.onCall(0).resolves([{ id: 1 }, { id: 2 }]);
      AuditLogStub.findAll.onCall(1).resolves([{ _COUNT: 12 }]);
      ...
      cmd.executeQuery = async function () {
        const data = await AuditLogStub.findAll();
        const count = await AuditLogStub.findAll();
        return {
          rows: data,
    ...
    it("should convert aggregations to float or int if convertAggregationsToNumbers exists", async () => {
      ...
      AuditLogStub.findAll.reset();
      AuditLogStub.findAll.resolves(data);
    ...
    it("should return first element directly if isAggregatedWithoutGroup", async () => {
      ...
      sandbox.stub(cmd, "getSelectList").returns(["total"]);
      sandbox.stub(cmd, "buildIncludes").returns(null);
      AuditLogStub.findAll.resolves([first]);
