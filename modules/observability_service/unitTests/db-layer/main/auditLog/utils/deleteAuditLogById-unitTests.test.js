...
describe("deleteAuditLogById module", () => {
  let sandbox;
  let deleteAuditLogById;
  ...
  beforeEach(() => {
    ...
    mockDocInstance = {
      ...
      getData: () => ({ id: fakeId, name: "Deleted AuditLog" }),
    };
    ...
    deleteAuditLogById = proxyquire(
      "../../../../../src/db-layer/main/AuditLog/utils/deleteAuditLogById",
      {
  ...
  describe("deleteAuditLogById", () => {
    it("should delete AuditLog and index removal in Elastic", async () => {
      const result = await deleteAuditLogById(fakeId);

      expect(result).to.deep.equal({ id: fakeId, name: "Deleted AuditLog" });
    ...
    it("should support object as ID input", async () => {
      ...
      AuditLogStub.findOne.resolves(localMock);

      const result = await deleteAuditLogById({ id: fakeId });
    ...
    it("should throw BadRequestError if no id is provided", async () => {
      try {
        await deleteAuditLogById(undefined);
    ...
    it("should throw NotFoundError if record is not found", async () => {
      ...
      try {
        await deleteAuditLogById(fakeId);
    ...
    it("should wrap internal errors with HttpServerError", async () => {
      AuditLogStub.findOne.rejects(new Error("unexpected error"));
    ...
    it("should wrap ElasticIndexer.deleteData() error with HttpServerError", async () => {
      ...
      const result = await deleteAuditLogById(fakeId).catch((err) => err);
    ...
    it("should accept numeric ID as valid input", async () => {
      ...
      const localMock = {
        id: numericId,
        update: sandbox.stub().resolves(),
        getData: () => ({ id: numericId, name: "Deleted AuditLog" }),
      };
      AuditLogStub.findOne.resolves(localMock);

      const result = await deleteAuditLogById(numericId);
      expect(result).to.deep.equal({ id: numericId, name: "Deleted AuditLog" });
      sinon.assert.calledOnce(localMock.update);
    });
  });
});
