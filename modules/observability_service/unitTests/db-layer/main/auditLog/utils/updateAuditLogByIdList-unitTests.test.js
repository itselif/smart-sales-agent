...
describe("updateAuditLogByIdList module", () => {
  let sandbox;
  let updateAuditLogByIdList;
  let AuditLogStub;
  ...
  beforeEach(() => {
    ...
    updateAuditLogByIdList = proxyquire(
      "../../../../../src/db-layer/main/AuditLog/utils/updateAuditLogByIdList",
      {
  ...
  describe("updateAuditLogByIdList", () => {
    ...
    it("should return empty list if update returns no rows", async () => {
      ...
      const result = await updateAuditLogByIdList(["id99"], {
        status: "inactive",
      });
    ...
    it("should return list with one id if only one record updated", async () => {
      AuditLogStub.update.resolves([1, [{ id: "id1" }]]);

      const result = await updateAuditLogByIdList(["id1"], { active: false });
    ...
    it("should throw HttpServerError if model update fails", async () => {
      ...
      try {
        await updateAuditLogByIdList(["id1"], { name: "err" });
    ...
    it("should call update with empty dataClause", async () => {
      await updateAuditLogByIdList(["id1"], {});
      sinon.assert.calledOnce(AuditLogStub.update);
    });
  });
});
