...
describe("updateAnomalyEventByIdList module", () => {
  let sandbox;
  let updateAnomalyEventByIdList;
  ...
  beforeEach(() => {
    ...
    updateAnomalyEventByIdList = proxyquire(
      "../../../../../src/db-layer/main/AnomalyEvent/utils/updateAnomalyEventByIdList",
      {
  ...
  describe("updateAnomalyEventByIdList", () => {
    ...
    it("should return empty list if update returns no rows", async () => {
      ...
      const result = await updateAnomalyEventByIdList(["id99"], {
        status: "inactive",
      });
    ...
    it("should return list with one id if only one record updated", async () => {
      AnomalyEventStub.update.resolves([1, [{ id: "id1" }]]);

      const result = await updateAnomalyEventByIdList(["id1"], {
        active: false,
      });
    ...
    it("should throw HttpServerError if model update fails", async () => {
      ...
      try {
        await updateAnomalyEventByIdList(["id1"], { name: "err" });
    ...
    it("should call update with empty dataClause", async () => {
      await updateAnomalyEventByIdList(["id1"], {});
      sinon.assert.calledOnce(AnomalyEventStub.update);
    });
  });
});
