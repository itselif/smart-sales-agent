...
describe("deleteAnomalyEventById module", () => {
  let sandbox;
  let deleteAnomalyEventById;
  let AnomalyEventStub, ElasticIndexerStub;
  let mockDocInstance;
  ...
  beforeEach(() => {
    ...
    mockDocInstance = {
      ...
      getData: () => ({ id: fakeId, name: "Deleted AnomalyEvent" }),
    };
    ...
    deleteAnomalyEventById = proxyquire(
      "../../../../../src/db-layer/main/AnomalyEvent/utils/deleteAnomalyEventById",
      {
  ...
  describe("deleteAnomalyEventById", () => {
    it("should delete AnomalyEvent and index removal in Elastic", async () => {
      const result = await deleteAnomalyEventById(fakeId);

      expect(result).to.deep.equal({
        id: fakeId,
        name: "Deleted AnomalyEvent",
      });
    ...
    it("should support object as ID input", async () => {
      ...
      AnomalyEventStub.findOne.resolves(localMock);

      const result = await deleteAnomalyEventById({ id: fakeId });
    ...
    it("should throw BadRequestError if no id is provided", async () => {
      try {
        await deleteAnomalyEventById(undefined);
    ...
    it("should throw NotFoundError if record is not found", async () => {
      ...
      try {
        await deleteAnomalyEventById(fakeId);
    ...
    it("should wrap internal errors with HttpServerError", async () => {
      AnomalyEventStub.findOne.rejects(new Error("unexpected error"));
    ...
    it("should wrap ElasticIndexer.deleteData() error with HttpServerError", async () => {
      ...
      const result = await deleteAnomalyEventById(fakeId).catch((err) => err);
    ...
    it("should accept numeric ID as valid input", async () => {
      ...
      const localMock = {
        id: numericId,
        update: sandbox.stub().resolves(),
        getData: () => ({ id: numericId, name: "Deleted AnomalyEvent" }),
      };
      AnomalyEventStub.findOne.resolves(localMock);

      const result = await deleteAnomalyEventById(numericId);
      expect(result).to.deep.equal({
        id: numericId,
        name: "Deleted AnomalyEvent",
      });
