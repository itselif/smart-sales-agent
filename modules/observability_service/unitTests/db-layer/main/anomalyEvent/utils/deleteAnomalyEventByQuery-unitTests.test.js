...
describe("deleteAnomalyEventByQuery module", () => {
  let sandbox;
  let deleteAnomalyEventByQuery;
  ...
  beforeEach(() => {
    ...
    deleteAnomalyEventByQuery = proxyquire(
      "../../../../../src/db-layer/main/AnomalyEvent/utils/deleteAnomalyEventByQuery",
      {
  ...
  describe("deleteAnomalyEventByQuery", () => {
    ...
    it("should throw BadRequestError if query is undefined", async () => {
      try {
        await deleteAnomalyEventByQuery(undefined);
    ...
    it("should throw BadRequestError if query is not an object", async () => {
      try {
        await deleteAnomalyEventByQuery("string");
        throw new Error("Expected BadRequestError");
      } catch (err) {
        expect(err.name).to.equal("HttpServerError");
        expect(err.details.name).to.equal("BadRequestError");
    ...
    it("should wrap model update() error in HttpServerError", async () => {
      ...
      try {
        await deleteAnomalyEventByQuery({ test: true });
    ...
    it("should still return mapped array even if getData returns undefined", async () => {
      ...
      AnomalyEventStub.update.resolves([2, partial]);

      const result = await deleteAnomalyEventByQuery({ any: "field" });
