...
describe("updateAnomalyEventById module", () => {
  let sandbox;
  let updateAnomalyEventById;
  let AnomalyEventStub, ElasticIndexerStub;

  const fakeId = "uuid-123";
  const fakeUpdatedData = { id: fakeId, name: "Updated AnomalyEvent" };
  ...
  beforeEach(() => {
    ...
    updateAnomalyEventById = proxyquire(
      "../../../../../src/db-layer/main/AnomalyEvent/utils/updateAnomalyEventById",
      {
  ...
  describe("updateAnomalyEventById", () => {
    it("should update record by direct ID", async () => {
      const result = await updateAnomalyEventById(fakeId, { name: "Updated" });
    ...
    it("should extract ID from dataClause if not explicitly provided", async () => {
      const result = await updateAnomalyEventById(undefined, {
        id: fakeId,
        name: "Updated",
      });
      expect(result).to.deep.equal(fakeUpdatedData);
    ...
    it("should support object-as-ID input", async () => {
      const result = await updateAnomalyEventById(
        { id: fakeId },
        { name: "Updated" },
      );
      expect(result).to.deep.equal(fakeUpdatedData);
    ...
    it("should throw BadRequestError if no ID is provided", async () => {
      try {
        await updateAnomalyEventById(undefined, {});
    ...
    it("should throw NotFoundError if no existing record is found", async () => {
      ...
      try {
        await updateAnomalyEventById(fakeId, { name: "Updated" });
        throw new Error("Expected error");
      } catch (err) {
        expect(err.name).to.equal("HttpServerError");
    ...
    it("should throw HttpServerError if findOne fails", async () => {
      AnomalyEventStub.findOne.rejects(new Error("DB fail"));
      try {
        await updateAnomalyEventById(fakeId, { name: "Updated" });
        throw new Error("Expected error");
      } catch (err) {
    ...
    it("should throw HttpServerError if update fails", async () => {
      AnomalyEventStub.update.rejects(new Error("update error"));
      try {
        await updateAnomalyEventById(fakeId, { name: "Updated" });
        throw new Error("Expected error");
      } catch (err) {
