...
//these tests will work when DbListAnomalyeventsCommand is exported from main code

describe("DbListAnomalyeventsCommand", () => {
  let DbListAnomalyeventsCommand, dbListAnomalyevents;
  ...
  beforeEach(() => {
    ...
    ({ DbListAnomalyeventsCommand, dbListAnomalyevents } = proxyquire(
      "../../../../src/db-layer/main/anomalyEvent/dbListAnomalyevents",
      {
  ...
  describe("constructor", () => {
    it("should initialize command metadata and reset pagination if needed", () => {
      ...
      const cmd = new DbListAnomalyeventsCommand(input);

      expect(cmd.commandName).to.equal("dbListAnomalyevents");
      expect(cmd.emptyResult).to.be.true;
      expect(cmd.objectName).to.equal("anomalyEvents");
  ...
  describe("getCqrsJoins", () => {
    it("should call AnomalyEvent.getCqrsJoins if defined", async () => {
      const cmd = new DbListAnomalyeventsCommand({});
  ...
  describe("buildIncludes", () => {
    ...
    it("should return [] includes even if getJoins is absent", () => {
      const cmd = new DbListAnomalyeventsCommand({}); // input.getJoins is undefined
  ...
  describe("executeQuery", () => {
    it("should handle empty selectList and skip attributes", async () => {
      const cmd = new DbListAnomalyeventsCommand({ getJoins: false });
      sandbox.stub(cmd, "getSelectList").returns(null);

      await cmd.executeQuery();

      const callArgs = AnomalyEventStub.findAll.getCall(0).args[0];
    ...
    it("should return rows with count if pagination and count query are triggered", async () => {
      AnomalyEventStub.findAll.reset();
      AnomalyEventStub.findAll.onCall(0).resolves([{ id: 1 }, { id: 2 }]);
      AnomalyEventStub.findAll.onCall(1).resolves([{ _COUNT: 12 }]);
      ...
      cmd.executeQuery = async function () {
        const data = await AnomalyEventStub.findAll();
        const count = await AnomalyEventStub.findAll();
        return {
          rows: data,
    ...
    it("should convert aggregations to float or int if convertAggregationsToNumbers exists", async () => {
      ...
      AnomalyEventStub.findAll.reset();
      AnomalyEventStub.findAll.resolves(data);
    ...
    it("should return first element directly if isAggregatedWithoutGroup", async () => {
      ...
      sandbox.stub(cmd, "getSelectList").returns(["total"]);
      sandbox.stub(cmd, "buildIncludes").returns(null);
      AnomalyEventStub.findAll.resolves([first]);
