...
describe("updateMetricDatapointById module", () => {
  let sandbox;
  let updateMetricDatapointById;
  let MetricDatapointStub, ElasticIndexerStub;

  const fakeId = "uuid-123";
  const fakeUpdatedData = { id: fakeId, name: "Updated MetricDatapoint" };
  ...
  beforeEach(() => {
    ...
    updateMetricDatapointById = proxyquire(
      "../../../../../src/db-layer/main/MetricDatapoint/utils/updateMetricDatapointById",
      {
  ...
  describe("updateMetricDatapointById", () => {
    it("should update record by direct ID", async () => {
      const result = await updateMetricDatapointById(fakeId, {
        name: "Updated",
      });
    ...
    it("should extract ID from dataClause if not explicitly provided", async () => {
      const result = await updateMetricDatapointById(undefined, {
        id: fakeId,
        name: "Updated",
      });
      expect(result).to.deep.equal(fakeUpdatedData);
    ...
    it("should support object-as-ID input", async () => {
      const result = await updateMetricDatapointById(
        { id: fakeId },
        { name: "Updated" },
      );
      expect(result).to.deep.equal(fakeUpdatedData);
    ...
    it("should throw BadRequestError if no ID is provided", async () => {
      try {
        await updateMetricDatapointById(undefined, {});
    ...
    it("should throw NotFoundError if no existing record is found", async () => {
      ...
      try {
        await updateMetricDatapointById(fakeId, { name: "Updated" });
        throw new Error("Expected error");
      } catch (err) {
        expect(err.name).to.equal("HttpServerError");
    ...
    it("should throw HttpServerError if findOne fails", async () => {
      MetricDatapointStub.findOne.rejects(new Error("DB fail"));
      try {
        await updateMetricDatapointById(fakeId, { name: "Updated" });
        throw new Error("Expected error");
      } catch (err) {
