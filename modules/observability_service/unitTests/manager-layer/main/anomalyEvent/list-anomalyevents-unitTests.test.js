...
describe("ListAnomalyEvents", () => {
  ...
  beforeEach(() => {
    ...
    ControllerClass = proxyquire(
      "../../../../src/manager-layer/main/AnomalyEvent/list-anomalyevents",
      {
  ...
  it("should initialize controller with correct base properties", () => {
    ...
    expect(instance.options.name).to.equal("listAnomalyEvents");
    expect(instance.options.controllerType).to.equal("rest");
    expect(instance.options.crudType).to.equal("getList");
    expect(instance.dataName).to.equal("anomalyEvents");
  });
