...
console.log("Starting salesai-observability-service service...");
...
const start = async () => {
  process.title = "node-salesai1-observability-service";
  ...
  try {
    ...
    await setRedisData(
      "salesai1-observability-service:start",
      today.toUTCString(),
    );
    console.log(
      "Service start datetime is written to Redis key salesai1-observability-service:start",
      today.toUTCString(),
    );
  } catch (err) {
  ...
  console.log(
    "salesai-observability-service is listening HTTP/REST port " +
...
/* starting the server salesai-observability-service .... */
start();
