...
class DbListMetricdatapointsCommand extends DBGetListSequelizeCommand {
  constructor(input) {
    super(input);
    this.commandName = "dbListMetricdatapoints";
    this.emptyResult = true;
    this.objectName = "metricDatapoints";
  ...
  async transposeResult() {
    for (const metricDatapoint of this.dbData.items) {
  ...
  async getCqrsJoins(item) {
    if (MetricDatapoint.getCqrsJoins) {
      await MetricDatapoint.getCqrsJoins(item);
    }
  }
  ...
  async executeQuery() {
    ...
    metricDatapoints = await MetricDatapoint.findAll(options);

    return metricDatapoints;
  }
}

const dbListMetricdatapoints = (input) => {
  const dbGetListCommand = new DbListMetricdatapointsCommand(input);
  return dbGetListCommand.execute();
};

module.exports = dbListMetricdatapoints;
