...
const indexDataToElastic = async (data) => {
  const elasticIndexer = new ElasticIndexer("metricDatapoint");
  await elasticIndexer.indexData(data);
};

const updateMetricDatapointById = async (id, dataClause) => {
  try {
    if (!id && dataClause.id) {
      id = dataClause.id;
    ...
    const existingDoc = await MetricDatapoint.findOne({
    ...
    const [rowsCount, [dbDoc]] = await MetricDatapoint.update(
      dataClause,
      options,
    );
    if (!dbDoc) {
...
module.exports = updateMetricDatapointById;
