...
class DbListAnomalyeventsCommand extends DBGetListSequelizeCommand {
  constructor(input) {
    super(input);
    this.commandName = "dbListAnomalyevents";
    this.emptyResult = true;
    this.objectName = "anomalyEvents";
  ...
  async transposeResult() {
    for (const anomalyEvent of this.dbData.items) {
  ...
  async getCqrsJoins(item) {
    if (AnomalyEvent.getCqrsJoins) {
      await AnomalyEvent.getCqrsJoins(item);
    }
  }
  ...
  async executeQuery() {
    ...
    anomalyEvents = await AnomalyEvent.findAll(options);

    return anomalyEvents;
  }
}

const dbListAnomalyevents = (input) => {
  const dbGetListCommand = new DbListAnomalyeventsCommand(input);
  return dbGetListCommand.execute();
};

module.exports = dbListAnomalyevents;
