...
const getAnomalyEventStatsByQuery = async (query, stats) => {
  const promises = [];
  const statLabels = [];
  try {
    ...
    for (const stat of stats) {
      ...
      if (stat === "count") {
        promises.push(AnomalyEvent.count({ where: queryWithSoftDelete }));
        statLabels.push("count");
      } else if (statParts.length == 2) {
        if (statParts[0] === "sum") {
          promises.push(
            AnomalyEvent.sum(statParts[1], { where: queryWithSoftDelete }),
          );
          statLabels.push("sum-" + statParts[1]);
        } else if (statParts[0] === "avg") {
          promises.push(
            AnomalyEvent.avg(statParts[1], { where: queryWithSoftDelete }),
          );
          statLabels.push("avg-" + statParts[1]);
        } else if (statParts[0] === "min") {
          promises.push(
            AnomalyEvent.min(statParts[1], { where: queryWithSoftDelete }),
          );
          statLabels.push("min-" + statParts[1]);
        } else if (statParts[0] === "max") {
          promises.push(
            AnomalyEvent.max(statParts[1], { where: queryWithSoftDelete }),
          );
    ...
    if (promises.length == 0) {
      return await AnomalyEvent.count({ where: queryWithSoftDelete });
...
module.exports = getAnomalyEventStatsByQuery;
