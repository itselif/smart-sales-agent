...
const indexDataToElastic = async (data) => {
  const elasticIndexer = new ElasticIndexer("anomalyEvent");
  await elasticIndexer.indexData(data);
};

const updateAnomalyEventById = async (id, dataClause) => {
  try {
    if (!id && dataClause.id) {
      id = dataClause.id;
    ...
    const existingDoc = await AnomalyEvent.findOne({
    ...
    const [rowsCount, [dbDoc]] = await AnomalyEvent.update(dataClause, options);
    if (!dbDoc) {
...
module.exports = updateAnomalyEventById;
