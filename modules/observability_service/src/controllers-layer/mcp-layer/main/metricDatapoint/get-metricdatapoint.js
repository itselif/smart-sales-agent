...
class GetMetricDatapointMcpController extends ObservabilityMcpController {
  constructor(params) {
    super("getMetricDatapoint", "getmetricdatapoint", params);
    this.dataName = "metricDatapoint";
    this.crudType = "get";
  }

  createApiManager() {
    return new GetMetricDatapointManager(this.request, "mcp");
  }
...
module.exports = (headers) => {
  return {
    ...
    parameters: GetMetricDatapointMcpController.getInputScheme(),
    controller: async (mcpParams) => {
      mcpParams.headers = headers;
      const getMetricDatapointMcpController =
        new GetMetricDatapointMcpController(mcpParams);
      try {
        const result = await getMetricDatapointMcpController.processRequest();
        //return GetMetricDatapointMcpController.getOutputSchema().parse(result);
        return {
          content: [
            {
              type: "text",
