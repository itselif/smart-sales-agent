...
class DeleteMetricDatapointMcpController extends ObservabilityMcpController {
  constructor(params) {
    super("deleteMetricDatapoint", "deletemetricdatapoint", params);
    this.dataName = "metricDatapoint";
    this.crudType = "delete";
  }

  createApiManager() {
    return new DeleteMetricDatapointManager(this.request, "mcp");
  }
...
module.exports = (headers) => {
  return {
    ...
    description:
      "Soft-delete a metric datapoint (e.g., for cleanup, retention, etc).",
    parameters: DeleteMetricDatapointMcpController.getInputScheme(),
    controller: async (mcpParams) => {
      mcpParams.headers = headers;
      const deleteMetricDatapointMcpController =
        new DeleteMetricDatapointMcpController(mcpParams);
      try {
        const result =
          await deleteMetricDatapointMcpController.processRequest();
        //return DeleteMetricDatapointMcpController.getOutputSchema().parse(result);
        return {
          content: [
            {
              type: "text",
