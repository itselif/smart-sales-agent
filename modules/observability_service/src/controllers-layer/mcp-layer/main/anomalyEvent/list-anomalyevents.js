...
class ListAnomalyEventsMcpController extends ObservabilityMcpController {
  constructor(params) {
    super("listAnomalyEvents", "listanomalyevents", params);
    this.dataName = "anomalyEvents";
    this.crudType = "getList";
  }

  createApiManager() {
    return new ListAnomalyEventsManager(this.request, "mcp");
  }
...
module.exports = (headers) => {
  return {
    ...
    description:
      "Paginated/filterable query of anomaly events by type, time, severity, entity, user and status.",
    parameters: ListAnomalyEventsMcpController.getInputScheme(),
    controller: async (mcpParams) => {
      mcpParams.headers = headers;
      const listAnomalyEventsMcpController = new ListAnomalyEventsMcpController(
        mcpParams,
      );
      try {
        const result = await listAnomalyEventsMcpController.processRequest();
        //return ListAnomalyEventsMcpController.getOutputSchema().parse(result);
        return {
          content: [
            {
              type: "text",
