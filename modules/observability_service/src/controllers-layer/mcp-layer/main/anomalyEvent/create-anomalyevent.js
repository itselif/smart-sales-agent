...
class CreateAnomalyEventMcpController extends ObservabilityMcpController {
  constructor(params) {
    super("createAnomalyEvent", "createanomalyevent", params);
    this.dataName = "anomalyEvent";
    this.crudType = "create";
  }

  createApiManager() {
    return new CreateAnomalyEventManager(this.request, "mcp");
  }
  ...
  static getInputScheme() {
    return {
      ...
      affectedUserId: z
        ...
        .uuid()
        .optional()
        .describe("User affected by anomaly (if different from trigger)."),
      ...
      severity: z
        .enum([])
        .describe("Severity of anomaly: 0=low, 1=medium, 2=high, 3=critical."),
...
module.exports = (headers) => {
  return {
    ...
    description:
      "Create and log a new anomaly event for compliance and investigation.",
    parameters: CreateAnomalyEventMcpController.getInputScheme(),
    controller: async (mcpParams) => {
      mcpParams.headers = headers;
      const createAnomalyEventMcpController =
        new CreateAnomalyEventMcpController(mcpParams);
      try {
        const result = await createAnomalyEventMcpController.processRequest();
        //return CreateAnomalyEventMcpController.getOutputSchema().parse(result);
        return {
          content: [
            {
              type: "text",
