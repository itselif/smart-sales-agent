...
class GetAnomalyEventMcpController extends ObservabilityMcpController {
  constructor(params) {
    super("getAnomalyEvent", "getanomalyevent", params);
    this.dataName = "anomalyEvent";
    this.crudType = "get";
  }

  createApiManager() {
    return new GetAnomalyEventManager(this.request, "mcp");
  }
...
module.exports = (headers) => {
  return {
    ...
    parameters: GetAnomalyEventMcpController.getInputScheme(),
    controller: async (mcpParams) => {
      mcpParams.headers = headers;
      const getAnomalyEventMcpController = new GetAnomalyEventMcpController(
        mcpParams,
      );
      try {
        const result = await getAnomalyEventMcpController.processRequest();
        //return GetAnomalyEventMcpController.getOutputSchema().parse(result);
        return {
          content: [
            {
              type: "text",
