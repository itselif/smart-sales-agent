...
describe("updateReportPolicyByQuery module", () => {
  let sandbox;
  let updateReportPolicyByQuery;
  ...
  beforeEach(() => {
    ...
    updateReportPolicyByQuery = proxyquire(
      "../../../../../src/db-layer/main/ReportPolicy/utils/updateReportPolicyByQuery",
      {
  ...
  describe("updateReportPolicyByQuery", () => {
    it("should update records matching query and return getData list", async () => {
      const result = await updateReportPolicyByQuery(fakeDataClause, fakeQuery);
      ...
      const [calledDataClause, calledOptions] =
        ReportPolicyStub.update.firstCall.args;
    ...
    it("should return list of undefineds if getData() returns undefined", async () => {
      ReportPolicyStub.update.resolves([
        2,
    ...
    it("should throw BadRequestError if query is undefined", async () => {
      try {
        await updateReportPolicyByQuery(fakeDataClause, undefined);
    ...
    it("should throw BadRequestError if query is not an object", async () => {
      try {
        await updateReportPolicyByQuery(fakeDataClause, "not-an-object");
        throw new Error("Expected error");
      } catch (err) {
        expect(err.name).to.equal("HttpServerError");
        expect(err.details.name).to.equal("BadRequestError");
    ...
    it("should throw HttpServerError if model update fails", async () => {
      ...
      try {
        await updateReportPolicyByQuery(fakeDataClause, fakeQuery);
        throw new Error("Expected error");
      } catch (err) {
        expect(err.name).to.equal("HttpServerError");
        expect(err.message).to.equal(
          "errMsg_dbErrorWhenUpdatingReportPolicyByQuery",
        );
    ...
    it("should accept empty dataClause and still process", async () => {
      ...
      const result = await updateReportPolicyByQuery({}, fakeQuery);
      expect(result).to.deep.equal([]);
    });
  });
});
