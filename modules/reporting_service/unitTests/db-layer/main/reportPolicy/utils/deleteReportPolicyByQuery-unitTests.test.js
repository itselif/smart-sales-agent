...
describe("deleteReportPolicyByQuery module", () => {
  let sandbox;
  let deleteReportPolicyByQuery;
  ...
  beforeEach(() => {
    ...
    deleteReportPolicyByQuery = proxyquire(
      "../../../../../src/db-layer/main/ReportPolicy/utils/deleteReportPolicyByQuery",
      {
  ...
  describe("deleteReportPolicyByQuery", () => {
    ...
    it("should throw BadRequestError if query is undefined", async () => {
      try {
        await deleteReportPolicyByQuery(undefined);
    ...
    it("should throw BadRequestError if query is not an object", async () => {
      try {
        await deleteReportPolicyByQuery("string");
        throw new Error("Expected BadRequestError");
      } catch (err) {
        expect(err.name).to.equal("HttpServerError");
        expect(err.details.name).to.equal("BadRequestError");
    ...
    it("should wrap model update() error in HttpServerError", async () => {
      ...
      try {
        await deleteReportPolicyByQuery({ test: true });
    ...
    it("should still return mapped array even if getData returns undefined", async () => {
      ...
      ReportPolicyStub.update.resolves([2, partial]);

      const result = await deleteReportPolicyByQuery({ any: "field" });
