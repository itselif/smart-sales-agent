...
describe("updateReportPolicyById module", () => {
  let sandbox;
  let updateReportPolicyById;
  let ReportPolicyStub, ElasticIndexerStub;

  const fakeId = "uuid-123";
  const fakeUpdatedData = { id: fakeId, name: "Updated ReportPolicy" };
  ...
  beforeEach(() => {
    ...
    updateReportPolicyById = proxyquire(
      "../../../../../src/db-layer/main/ReportPolicy/utils/updateReportPolicyById",
      {
  ...
  describe("updateReportPolicyById", () => {
    it("should update record by direct ID", async () => {
      const result = await updateReportPolicyById(fakeId, { name: "Updated" });
    ...
    it("should extract ID from dataClause if not explicitly provided", async () => {
      const result = await updateReportPolicyById(undefined, {
        id: fakeId,
        name: "Updated",
      });
      expect(result).to.deep.equal(fakeUpdatedData);
    ...
    it("should support object-as-ID input", async () => {
      const result = await updateReportPolicyById(
        { id: fakeId },
        { name: "Updated" },
      );
      expect(result).to.deep.equal(fakeUpdatedData);
    ...
    it("should throw BadRequestError if no ID is provided", async () => {
      try {
        await updateReportPolicyById(undefined, {});
    ...
    it("should throw NotFoundError if no existing record is found", async () => {
      ...
      try {
        await updateReportPolicyById(fakeId, { name: "Updated" });
        throw new Error("Expected error");
      } catch (err) {
        expect(err.name).to.equal("HttpServerError");
    ...
    it("should throw HttpServerError if findOne fails", async () => {
      ReportPolicyStub.findOne.rejects(new Error("DB fail"));
      try {
        await updateReportPolicyById(fakeId, { name: "Updated" });
        throw new Error("Expected error");
      } catch (err) {
    ...
    it("should throw HttpServerError if update fails", async () => {
      ReportPolicyStub.update.rejects(new Error("update error"));
      try {
        await updateReportPolicyById(fakeId, { name: "Updated" });
        throw new Error("Expected error");
      } catch (err) {
