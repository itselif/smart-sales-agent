...
describe("updateReportFileByIdList module", () => {
  let sandbox;
  let updateReportFileByIdList;
  let ReportFileStub;
  ...
  beforeEach(() => {
    ...
    updateReportFileByIdList = proxyquire(
      "../../../../../src/db-layer/main/ReportFile/utils/updateReportFileByIdList",
      {
  ...
  describe("updateReportFileByIdList", () => {
    ...
    it("should return empty list if update returns no rows", async () => {
      ...
      const result = await updateReportFileByIdList(["id99"], {
        status: "inactive",
      });
    ...
    it("should return list with one id if only one record updated", async () => {
      ReportFileStub.update.resolves([1, [{ id: "id1" }]]);

      const result = await updateReportFileByIdList(["id1"], { active: false });
    ...
    it("should throw HttpServerError if model update fails", async () => {
      ...
      try {
        await updateReportFileByIdList(["id1"], { name: "err" });
    ...
    it("should call update with empty dataClause", async () => {
      await updateReportFileByIdList(["id1"], {});
      sinon.assert.calledOnce(ReportFileStub.update);
    });
  });
});
