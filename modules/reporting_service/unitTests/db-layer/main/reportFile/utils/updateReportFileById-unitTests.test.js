...
describe("updateReportFileById module", () => {
  let sandbox;
  let updateReportFileById;
  let ReportFileStub, ElasticIndexerStub;

  const fakeId = "uuid-123";
  const fakeUpdatedData = { id: fakeId, name: "Updated ReportFile" };
  ...
  beforeEach(() => {
    ...
    updateReportFileById = proxyquire(
      "../../../../../src/db-layer/main/ReportFile/utils/updateReportFileById",
      {
  ...
  describe("updateReportFileById", () => {
    it("should update record by direct ID", async () => {
      const result = await updateReportFileById(fakeId, { name: "Updated" });
    ...
    it("should extract ID from dataClause if not explicitly provided", async () => {
      const result = await updateReportFileById(undefined, {
        id: fakeId,
        name: "Updated",
      });
      expect(result).to.deep.equal(fakeUpdatedData);
    ...
    it("should support object-as-ID input", async () => {
      const result = await updateReportFileById(
        { id: fakeId },
        { name: "Updated" },
      );
      expect(result).to.deep.equal(fakeUpdatedData);
    ...
    it("should throw BadRequestError if no ID is provided", async () => {
      try {
        await updateReportFileById(undefined, {});
    ...
    it("should throw NotFoundError if no existing record is found", async () => {
      ...
      try {
        await updateReportFileById(fakeId, { name: "Updated" });
        throw new Error("Expected error");
      } catch (err) {
        expect(err.name).to.equal("HttpServerError");
    ...
    it("should throw HttpServerError if findOne fails", async () => {
      ReportFileStub.findOne.rejects(new Error("DB fail"));
      try {
        await updateReportFileById(fakeId, { name: "Updated" });
        throw new Error("Expected error");
      } catch (err) {
