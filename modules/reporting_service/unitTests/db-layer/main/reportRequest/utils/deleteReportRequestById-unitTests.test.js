...
describe("deleteReportRequestById module", () => {
  let sandbox;
  let deleteReportRequestById;
  let ReportRequestStub, ElasticIndexerStub;
  let mockDocInstance;
  ...
  beforeEach(() => {
    ...
    mockDocInstance = {
      ...
      getData: () => ({ id: fakeId, name: "Deleted ReportRequest" }),
    };
    ...
    deleteReportRequestById = proxyquire(
      "../../../../../src/db-layer/main/ReportRequest/utils/deleteReportRequestById",
      {
  ...
  describe("deleteReportRequestById", () => {
    it("should delete ReportRequest and index removal in Elastic", async () => {
      const result = await deleteReportRequestById(fakeId);

      expect(result).to.deep.equal({
        id: fakeId,
        name: "Deleted ReportRequest",
      });
    ...
    it("should support object as ID input", async () => {
      ...
      ReportRequestStub.findOne.resolves(localMock);

      const result = await deleteReportRequestById({ id: fakeId });
    ...
    it("should throw BadRequestError if no id is provided", async () => {
      try {
        await deleteReportRequestById(undefined);
    ...
    it("should throw NotFoundError if record is not found", async () => {
      ...
      try {
        await deleteReportRequestById(fakeId);
    ...
    it("should wrap internal errors with HttpServerError", async () => {
      ReportRequestStub.findOne.rejects(new Error("unexpected error"));
    ...
    it("should wrap ElasticIndexer.deleteData() error with HttpServerError", async () => {
      ...
      const result = await deleteReportRequestById(fakeId).catch((err) => err);
    ...
    it("should accept numeric ID as valid input", async () => {
      ...
      const localMock = {
        id: numericId,
        update: sandbox.stub().resolves(),
        getData: () => ({ id: numericId, name: "Deleted ReportRequest" }),
      };
      ReportRequestStub.findOne.resolves(localMock);

      const result = await deleteReportRequestById(numericId);
      expect(result).to.deep.equal({
        id: numericId,
        name: "Deleted ReportRequest",
      });
