...
describe("updateReportRequestById module", () => {
  let sandbox;
  let updateReportRequestById;
  let ReportRequestStub, ElasticIndexerStub;

  const fakeId = "uuid-123";
  const fakeUpdatedData = { id: fakeId, name: "Updated ReportRequest" };
  ...
  beforeEach(() => {
    ...
    updateReportRequestById = proxyquire(
      "../../../../../src/db-layer/main/ReportRequest/utils/updateReportRequestById",
      {
  ...
  describe("updateReportRequestById", () => {
    it("should update record by direct ID", async () => {
      const result = await updateReportRequestById(fakeId, { name: "Updated" });
    ...
    it("should extract ID from dataClause if not explicitly provided", async () => {
      const result = await updateReportRequestById(undefined, {
        id: fakeId,
        name: "Updated",
      });
      expect(result).to.deep.equal(fakeUpdatedData);
    ...
    it("should support object-as-ID input", async () => {
      const result = await updateReportRequestById(
        { id: fakeId },
        { name: "Updated" },
      );
      expect(result).to.deep.equal(fakeUpdatedData);
    ...
    it("should throw BadRequestError if no ID is provided", async () => {
      try {
        await updateReportRequestById(undefined, {});
    ...
    it("should throw NotFoundError if no existing record is found", async () => {
      ...
      try {
        await updateReportRequestById(fakeId, { name: "Updated" });
        throw new Error("Expected error");
      } catch (err) {
        expect(err.name).to.equal("HttpServerError");
    ...
    it("should throw HttpServerError if findOne fails", async () => {
      ReportRequestStub.findOne.rejects(new Error("DB fail"));
      try {
        await updateReportRequestById(fakeId, { name: "Updated" });
        throw new Error("Expected error");
      } catch (err) {
