...
describe("DeleteReportPolicy", () => {
  ...
  beforeEach(() => {
    ...
    ControllerClass = proxyquire(
      "../../../../src/manager-layer/main/ReportPolicy/delete-reportpolicy",
      {
  ...
  it("should initialize controller with correct base properties", () => {
    ...
    expect(instance.options.name).to.equal("deleteReportPolicy");
    expect(instance.options.controllerType).to.equal("rest");
    expect(instance.options.crudType).to.equal("delete");
    expect(instance.dataName).to.equal("reportPolicy");
  });
