...
class CreateReportRequestMcpController extends ReportingMcpController {
  constructor(params) {
    super("createReportRequest", "createreportrequest", params);
    this.dataName = "reportRequest";
    this.crudType = "create";
  }

  createApiManager() {
    return new CreateReportRequestManager(this.request, "mcp");
  }
  ...
  static getInputScheme() {
    return {
      ...
      productIds: z
        ...
        .max(255)
        .optional()
        .describe("SKUs or IDs of products relevant to the report, optional."),
...
module.exports = (headers) => {
  return {
    ...
    parameters: CreateReportRequestMcpController.getInputScheme(),
    controller: async (mcpParams) => {
      mcpParams.headers = headers;
      const createReportRequestMcpController =
        new CreateReportRequestMcpController(mcpParams);
      try {
        const result = await createReportRequestMcpController.processRequest();
        //return CreateReportRequestMcpController.getOutputSchema().parse(result);
        return {
          content: [
            {
              type: "text",
