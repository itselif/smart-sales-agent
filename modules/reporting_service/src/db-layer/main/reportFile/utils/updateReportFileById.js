...
const indexDataToElastic = async (data) => {
  const elasticIndexer = new ElasticIndexer("reportFile");
  await elasticIndexer.indexData(data);
};

const updateReportFileById = async (id, dataClause) => {
  try {
    if (!id && dataClause.id) {
      id = dataClause.id;
    ...
    const existingDoc = await ReportFile.findOne({
    ...
    const [rowsCount, [dbDoc]] = await ReportFile.update(dataClause, options);
    if (!dbDoc) {
...
module.exports = updateReportFileById;
