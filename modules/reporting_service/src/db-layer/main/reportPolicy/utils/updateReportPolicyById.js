...
const indexDataToElastic = async (data) => {
  const elasticIndexer = new ElasticIndexer("reportPolicy");
  await elasticIndexer.indexData(data);
};

const updateReportPolicyById = async (id, dataClause) => {
  try {
    if (!id && dataClause.id) {
      id = dataClause.id;
    ...
    const existingDoc = await ReportPolicy.findOne({
    ...
    const [rowsCount, [dbDoc]] = await ReportPolicy.update(dataClause, options);
    if (!dbDoc) {
...
module.exports = updateReportPolicyById;
