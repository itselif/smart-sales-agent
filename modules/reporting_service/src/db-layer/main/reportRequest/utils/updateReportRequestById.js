...
const indexDataToElastic = async (data) => {
  const elasticIndexer = new ElasticIndexer("reportRequest");
  await elasticIndexer.indexData(data);
};

const updateReportRequestById = async (id, dataClause) => {
  try {
    if (!id && dataClause.id) {
      id = dataClause.id;
    ...
    const existingDoc = await ReportRequest.findOne({
    ...
    const [rowsCount, [dbDoc]] = await ReportRequest.update(
      dataClause,
      options,
    );
    if (!dbDoc) {
...
module.exports = updateReportRequestById;
