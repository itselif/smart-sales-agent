...
console.log("Starting salesai-reporting-service service...");
...
const start = async () => {
  process.title = "node-salesai1-reporting-service";
  ...
  try {
    ...
    await setRedisData("salesai1-reporting-service:start", today.toUTCString());
    console.log(
      "Service start datetime is written to Redis key salesai1-reporting-service:start",
      today.toUTCString(),
    );
  } catch (err) {
  ...
  console.log(
    "salesai-reporting-service is listening HTTP/REST port " +
...
/* starting the server salesai-reporting-service .... */
start();
