...
describe("updateOpenApiSchemaById module", () => {
  let sandbox;
  let updateOpenApiSchemaById;
  let OpenApiSchemaStub, ElasticIndexerStub;

  const fakeId = "uuid-123";
  const fakeUpdatedData = { id: fakeId, name: "Updated OpenApiSchema" };
  ...
  beforeEach(() => {
    ...
    updateOpenApiSchemaById = proxyquire(
      "../../../../../src/db-layer/main/OpenApiSchema/utils/updateOpenApiSchemaById",
      {
  ...
  describe("updateOpenApiSchemaById", () => {
    it("should update record by direct ID", async () => {
      const result = await updateOpenApiSchemaById(fakeId, { name: "Updated" });
    ...
    it("should extract ID from dataClause if not explicitly provided", async () => {
      const result = await updateOpenApiSchemaById(undefined, {
        id: fakeId,
        name: "Updated",
      });
      expect(result).to.deep.equal(fakeUpdatedData);
    ...
    it("should support object-as-ID input", async () => {
      const result = await updateOpenApiSchemaById(
        { id: fakeId },
        { name: "Updated" },
      );
      expect(result).to.deep.equal(fakeUpdatedData);
    ...
    it("should throw BadRequestError if no ID is provided", async () => {
      try {
        await updateOpenApiSchemaById(undefined, {});
    ...
    it("should throw NotFoundError if no existing record is found", async () => {
      ...
      try {
        await updateOpenApiSchemaById(fakeId, { name: "Updated" });
        throw new Error("Expected error");
      } catch (err) {
        expect(err.name).to.equal("HttpServerError");
    ...
    it("should throw HttpServerError if findOne fails", async () => {
      OpenApiSchemaStub.findOne.rejects(new Error("DB fail"));
      try {
        await updateOpenApiSchemaById(fakeId, { name: "Updated" });
        throw new Error("Expected error");
      } catch (err) {
    ...
    it("should throw HttpServerError if update fails", async () => {
      OpenApiSchemaStub.update.rejects(new Error("update error"));
      try {
        await updateOpenApiSchemaById(fakeId, { name: "Updated" });
        throw new Error("Expected error");
      } catch (err) {
