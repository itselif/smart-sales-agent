...
//these tests will work when DbListOpenapischemasCommand is exported from main code

describe("DbListOpenapischemasCommand", () => {
  let DbListOpenapischemasCommand, dbListOpenapischemas;
  ...
  beforeEach(() => {
    ...
    ({ DbListOpenapischemasCommand, dbListOpenapischemas } = proxyquire(
      "../../../../src/db-layer/main/openApiSchema/dbListOpenapischemas",
      {
  ...
  describe("constructor", () => {
    it("should initialize command metadata and reset pagination if needed", () => {
      ...
      const cmd = new DbListOpenapischemasCommand(input);

      expect(cmd.commandName).to.equal("dbListOpenapischemas");
      expect(cmd.emptyResult).to.be.true;
      expect(cmd.objectName).to.equal("openApiSchemas");
  ...
  describe("getCqrsJoins", () => {
    it("should call OpenApiSchema.getCqrsJoins if defined", async () => {
      const cmd = new DbListOpenapischemasCommand({});
  ...
  describe("buildIncludes", () => {
    ...
    it("should return [] includes even if getJoins is absent", () => {
      const cmd = new DbListOpenapischemasCommand({}); // input.getJoins is undefined
  ...
  describe("executeQuery", () => {
    it("should handle empty selectList and skip attributes", async () => {
      const cmd = new DbListOpenapischemasCommand({ getJoins: false });
      sandbox.stub(cmd, "getSelectList").returns(null);

      await cmd.executeQuery();

      const callArgs = OpenApiSchemaStub.findAll.getCall(0).args[0];
    ...
    it("should return rows with count if pagination and count query are triggered", async () => {
      OpenApiSchemaStub.findAll.reset();
      OpenApiSchemaStub.findAll.onCall(0).resolves([{ id: 1 }, { id: 2 }]);
      OpenApiSchemaStub.findAll.onCall(1).resolves([{ _COUNT: 12 }]);
      ...
      cmd.executeQuery = async function () {
        const data = await OpenApiSchemaStub.findAll();
        const count = await OpenApiSchemaStub.findAll();
        return {
          rows: data,
    ...
    it("should convert aggregations to float or int if convertAggregationsToNumbers exists", async () => {
      ...
      OpenApiSchemaStub.findAll.reset();
      OpenApiSchemaStub.findAll.resolves(data);
    ...
    it("should return first element directly if isAggregatedWithoutGroup", async () => {
      ...
      sandbox.stub(cmd, "getSelectList").returns(["total"]);
      sandbox.stub(cmd, "buildIncludes").returns(null);
      OpenApiSchemaStub.findAll.resolves([first]);
