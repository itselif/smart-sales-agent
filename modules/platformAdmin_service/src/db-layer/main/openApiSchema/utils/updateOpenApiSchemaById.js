...
const indexDataToElastic = async (data) => {
  const elasticIndexer = new ElasticIndexer("openApiSchema");
  await elasticIndexer.indexData(data);
};

const updateOpenApiSchemaById = async (id, dataClause) => {
  try {
    if (!id && dataClause.id) {
      id = dataClause.id;
    ...
    const existingDoc = await OpenApiSchema.findOne({
    ...
    const [rowsCount, [dbDoc]] = await OpenApiSchema.update(
      dataClause,
      options,
    );
    if (!dbDoc) {
...
module.exports = updateOpenApiSchemaById;
