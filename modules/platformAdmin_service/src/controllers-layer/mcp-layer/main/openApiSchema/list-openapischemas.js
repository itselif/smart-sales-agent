...
class ListOpenApiSchemasMcpController extends PlatformAdminMcpController {
  constructor(params) {
    super("listOpenApiSchemas", "listopenapischemas", params);
    this.dataName = "openApiSchemas";
    this.crudType = "getList";
  }

  createApiManager() {
    return new ListOpenApiSchemasManager(this.request, "mcp");
  }
...
module.exports = (headers) => {
  return {
    ...
    parameters: ListOpenApiSchemasMcpController.getInputScheme(),
    controller: async (mcpParams) => {
      mcpParams.headers = headers;
      const listOpenApiSchemasMcpController =
        new ListOpenApiSchemasMcpController(mcpParams);
      try {
        const result = await listOpenApiSchemasMcpController.processRequest();
        //return ListOpenApiSchemasMcpController.getOutputSchema().parse(result);
        return {
          content: [
            {
              type: "text",
