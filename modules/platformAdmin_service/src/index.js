...
console.log("Starting salesai-platformadmin-service service...");
...
const start = async () => {
  process.title = "node-salesai1-platformadmin-service";
  ...
  try {
    ...
    await setRedisData(
      "salesai1-platformadmin-service:start",
      today.toUTCString(),
    );
    console.log(
      "Service start datetime is written to Redis key salesai1-platformadmin-service:start",
      today.toUTCString(),
    );
  } catch (err) {
  ...
  console.log(
    "salesai-platformadmin-service is listening HTTP/REST port " +
...
/* starting the server salesai-platformadmin-service .... */
start();
