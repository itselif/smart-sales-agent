...
describe("DeleteSaleTransactionHistory", () => {
  let ControllerClass;
  let req;
  ...
  beforeEach(() => {
    ...
    ControllerClass = proxyquire(
      "../../../../src/manager-layer/main/SaleTransactionHistory/delete-saletransactionhistory",
      {
  ...
  it("should initialize controller with correct base properties", () => {
    ...
    expect(instance.options.name).to.equal("deleteSaleTransactionHistory");
    expect(instance.options.controllerType).to.equal("rest");
    expect(instance.options.crudType).to.equal("delete");
    expect(instance.dataName).to.equal("saleTransactionHistory");
  });
