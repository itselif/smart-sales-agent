...
describe("deleteSaleTransactionById module", () => {
  let sandbox;
  let deleteSaleTransactionById;
  ...
  beforeEach(() => {
    ...
    mockDocInstance = {
      ...
      getData: () => ({ id: fakeId, name: "Deleted SaleTransaction" }),
    };
    ...
    deleteSaleTransactionById = proxyquire(
      "../../../../../src/db-layer/main/SaleTransaction/utils/deleteSaleTransactionById",
      {
  ...
  describe("deleteSaleTransactionById", () => {
    it("should delete SaleTransaction and index removal in Elastic", async () => {
      const result = await deleteSaleTransactionById(fakeId);

      expect(result).to.deep.equal({
        id: fakeId,
        name: "Deleted SaleTransaction",
      });
    ...
    it("should support object as ID input", async () => {
      ...
      SaleTransactionStub.findOne.resolves(localMock);

      const result = await deleteSaleTransactionById({ id: fakeId });
    ...
    it("should throw BadRequestError if no id is provided", async () => {
      try {
        await deleteSaleTransactionById(undefined);
    ...
    it("should throw NotFoundError if record is not found", async () => {
      ...
      try {
        await deleteSaleTransactionById(fakeId);
    ...
    it("should wrap internal errors with HttpServerError", async () => {
      SaleTransactionStub.findOne.rejects(new Error("unexpected error"));
    ...
    it("should wrap ElasticIndexer.deleteData() error with HttpServerError", async () => {
      ...
      const result = await deleteSaleTransactionById(fakeId).catch(
        (err) => err,
      );
    ...
    it("should accept numeric ID as valid input", async () => {
      ...
      const localMock = {
        id: numericId,
        update: sandbox.stub().resolves(),
        getData: () => ({ id: numericId, name: "Deleted SaleTransaction" }),
      };
      SaleTransactionStub.findOne.resolves(localMock);

      const result = await deleteSaleTransactionById(numericId);
      expect(result).to.deep.equal({
        id: numericId,
        name: "Deleted SaleTransaction",
      });
