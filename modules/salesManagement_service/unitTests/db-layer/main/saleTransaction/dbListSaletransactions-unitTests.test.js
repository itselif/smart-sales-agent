...
//these tests will work when DbListSaletransactionsCommand is exported from main code

describe("DbListSaletransactionsCommand", () => {
  let DbListSaletransactionsCommand, dbListSaletransactions;
  ...
  beforeEach(() => {
    ...
    ({ DbListSaletransactionsCommand, dbListSaletransactions } = proxyquire(
      "../../../../src/db-layer/main/saleTransaction/dbListSaletransactions",
      {
  ...
  describe("constructor", () => {
    it("should initialize command metadata and reset pagination if needed", () => {
      ...
      const cmd = new DbListSaletransactionsCommand(input);

      expect(cmd.commandName).to.equal("dbListSaletransactions");
      expect(cmd.emptyResult).to.be.true;
      expect(cmd.objectName).to.equal("saleTransactions");
  ...
  describe("getCqrsJoins", () => {
    it("should call SaleTransaction.getCqrsJoins if defined", async () => {
      const cmd = new DbListSaletransactionsCommand({});
  ...
  describe("buildIncludes", () => {
    ...
    it("should return [] includes even if getJoins is absent", () => {
      const cmd = new DbListSaletransactionsCommand({}); // input.getJoins is undefined
  ...
  describe("executeQuery", () => {
    it("should handle empty selectList and skip attributes", async () => {
      const cmd = new DbListSaletransactionsCommand({ getJoins: false });
      sandbox.stub(cmd, "getSelectList").returns(null);

      await cmd.executeQuery();

      const callArgs = SaleTransactionStub.findAll.getCall(0).args[0];
    ...
    it("should return rows with count if pagination and count query are triggered", async () => {
      SaleTransactionStub.findAll.reset();
      SaleTransactionStub.findAll.onCall(0).resolves([{ id: 1 }, { id: 2 }]);
      SaleTransactionStub.findAll.onCall(1).resolves([{ _COUNT: 12 }]);
      ...
      cmd.executeQuery = async function () {
        const data = await SaleTransactionStub.findAll();
        const count = await SaleTransactionStub.findAll();
        return {
          rows: data,
    ...
    it("should convert aggregations to float or int if convertAggregationsToNumbers exists", async () => {
      ...
      SaleTransactionStub.findAll.reset();
      SaleTransactionStub.findAll.resolves(data);
    ...
    it("should return first element directly if isAggregatedWithoutGroup", async () => {
      ...
      sandbox.stub(cmd, "getSelectList").returns(["total"]);
      sandbox.stub(cmd, "buildIncludes").returns(null);
      SaleTransactionStub.findAll.resolves([first]);
