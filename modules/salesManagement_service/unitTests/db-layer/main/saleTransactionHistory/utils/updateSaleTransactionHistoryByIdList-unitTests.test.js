...
describe("updateSaleTransactionHistoryByIdList module", () => {
  let sandbox;
  let updateSaleTransactionHistoryByIdList;
  ...
  beforeEach(() => {
    ...
    updateSaleTransactionHistoryByIdList = proxyquire(
      "../../../../../src/db-layer/main/SaleTransactionHistory/utils/updateSaleTransactionHistoryByIdList",
      {
  ...
  describe("updateSaleTransactionHistoryByIdList", () => {
    it("should return list of updated IDs if update is successful", async () => {
      const result = await updateSaleTransactionHistoryByIdList(fakeIdList, {
        name: "Updated",
      });

      expect(result).to.deep.equal(["id1", "id2"]);
      sinon.assert.calledOnce(SaleTransactionHistoryStub.update);
      const args = SaleTransactionHistoryStub.update.getCall(0).args;
    ...
    it("should return empty list if update returns no rows", async () => {
      ...
      const result = await updateSaleTransactionHistoryByIdList(["id99"], {
        status: "inactive",
      });
    ...
    it("should return list with one id if only one record updated", async () => {
      SaleTransactionHistoryStub.update.resolves([1, [{ id: "id1" }]]);

      const result = await updateSaleTransactionHistoryByIdList(["id1"], {
        active: false,
      });
    ...
    it("should throw HttpServerError if model update fails", async () => {
      ...
      try {
        await updateSaleTransactionHistoryByIdList(["id1"], { name: "err" });
    ...
    it("should call update with empty dataClause", async () => {
      await updateSaleTransactionHistoryByIdList(["id1"], {});
      sinon.assert.calledOnce(SaleTransactionHistoryStub.update);
    });
  });
});
