...
describe("updateSaleTransactionHistoryById module", () => {
  let sandbox;
  let updateSaleTransactionHistoryById;
  let SaleTransactionHistoryStub, ElasticIndexerStub;

  const fakeId = "uuid-123";
  const fakeUpdatedData = {
    id: fakeId,
    name: "Updated SaleTransactionHistory",
  };
  ...
  beforeEach(() => {
    ...
    updateSaleTransactionHistoryById = proxyquire(
      "../../../../../src/db-layer/main/SaleTransactionHistory/utils/updateSaleTransactionHistoryById",
      {
  ...
  describe("updateSaleTransactionHistoryById", () => {
    it("should update record by direct ID", async () => {
      const result = await updateSaleTransactionHistoryById(fakeId, {
        name: "Updated",
      });
    ...
    it("should extract ID from dataClause if not explicitly provided", async () => {
      const result = await updateSaleTransactionHistoryById(undefined, {
        id: fakeId,
        name: "Updated",
      });
      expect(result).to.deep.equal(fakeUpdatedData);
    ...
    it("should support object-as-ID input", async () => {
      const result = await updateSaleTransactionHistoryById(
        { id: fakeId },
        { name: "Updated" },
      );
      expect(result).to.deep.equal(fakeUpdatedData);
    ...
    it("should throw BadRequestError if no ID is provided", async () => {
      try {
        await updateSaleTransactionHistoryById(undefined, {});
    ...
    it("should throw NotFoundError if no existing record is found", async () => {
      ...
      try {
        await updateSaleTransactionHistoryById(fakeId, { name: "Updated" });
        throw new Error("Expected error");
      } catch (err) {
        expect(err.name).to.equal("HttpServerError");
    ...
    it("should throw HttpServerError if findOne fails", async () => {
      SaleTransactionHistoryStub.findOne.rejects(new Error("DB fail"));
      try {
        await updateSaleTransactionHistoryById(fakeId, { name: "Updated" });
        throw new Error("Expected error");
      } catch (err) {
