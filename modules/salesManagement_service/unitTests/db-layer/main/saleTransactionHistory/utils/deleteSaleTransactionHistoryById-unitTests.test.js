...
describe("deleteSaleTransactionHistoryById module", () => {
  let sandbox;
  let deleteSaleTransactionHistoryById;
  let SaleTransactionHistoryStub, ElasticIndexerStub;
  let mockDocInstance;
  ...
  beforeEach(() => {
    ...
    mockDocInstance = {
      ...
      getData: () => ({ id: fakeId, name: "Deleted SaleTransactionHistory" }),
    };
    ...
    deleteSaleTransactionHistoryById = proxyquire(
      "../../../../../src/db-layer/main/SaleTransactionHistory/utils/deleteSaleTransactionHistoryById",
      {
  ...
  describe("deleteSaleTransactionHistoryById", () => {
    it("should delete SaleTransactionHistory and index removal in Elastic", async () => {
      const result = await deleteSaleTransactionHistoryById(fakeId);

      expect(result).to.deep.equal({
        id: fakeId,
        name: "Deleted SaleTransactionHistory",
      });
    ...
    it("should support object as ID input", async () => {
      ...
      SaleTransactionHistoryStub.findOne.resolves(localMock);

      const result = await deleteSaleTransactionHistoryById({ id: fakeId });
    ...
    it("should throw BadRequestError if no id is provided", async () => {
      try {
        await deleteSaleTransactionHistoryById(undefined);
    ...
    it("should throw NotFoundError if record is not found", async () => {
      ...
      try {
        await deleteSaleTransactionHistoryById(fakeId);
    ...
    it("should wrap internal errors with HttpServerError", async () => {
      SaleTransactionHistoryStub.findOne.rejects(new Error("unexpected error"));
    ...
    it("should wrap ElasticIndexer.deleteData() error with HttpServerError", async () => {
      ...
      const result = await deleteSaleTransactionHistoryById(fakeId).catch(
        (err) => err,
      );
    ...
    it("should accept numeric ID as valid input", async () => {
      ...
      const result = await deleteSaleTransactionHistoryById(numericId);
      expect(result).to.deep.equal({
        id: numericId,
        name: "Deleted SaleTransactionHistory",
      });
