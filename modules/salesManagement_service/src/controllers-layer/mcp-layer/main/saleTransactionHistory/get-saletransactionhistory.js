...
class GetSaleTransactionHistoryMcpController extends SalesManagementMcpController {
  constructor(params) {
    super("getSaleTransactionHistory", "getsaletransactionhistory",params);
    this.dataName = 'saleTransactionHistory';
    this.crudType = "get";
  }

  createApiManager() {
    return new GetSaleTransactionHistoryManager(this.request,"mcp");
  }
 ...
 static getInputScheme() {
    ...
    accessToken : z.string().optional().describe("The access token which is returned from a login request or given by user. This access token will override if there is any bearer or OAuth token in the mcp client. If not given the request will be made with the system (bearer or OAuth) token. For public routes you dont need to deifne any access token."),
                        saleTransactionHistoryId : z.string().uuid().describe("This id paremeter is used to select the required data object that is queried"),
...
return {
  ...
  parameters: GetSaleTransactionHistoryMcpController.getInputScheme(),
  controller:  async (mcpParams) => {
    mcpParams.headers =  headers;
  const getSaleTransactionHistoryMcpController =  new GetSaleTransactionHistoryMcpController(mcpParams);
  try {
    const result = await getSaleTransactionHistoryMcpController.processRequest();
    //return GetSaleTransactionHistoryMcpController.getOutputSchema().parse(result);  
    return {
      content: [
        {
          type: "text",
