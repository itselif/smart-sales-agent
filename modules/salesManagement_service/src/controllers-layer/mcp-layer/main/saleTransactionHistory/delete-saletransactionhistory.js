...
class DeleteSaleTransactionHistoryMcpController extends SalesManagementMcpController {
  constructor(params) {
    super("deleteSaleTransactionHistory", "deletesaletransactionhistory",params);
    this.dataName = 'saleTransactionHistory';
    this.crudType = "delete";
  }

  createApiManager() {
    return new DeleteSaleTransactionHistoryManager(this.request,"mcp");
  }
 ...
 static getInputScheme() {
    ...
    accessToken : z.string().optional().describe("The access token which is returned from a login request or given by user. This access token will override if there is any bearer or OAuth token in the mcp client. If not given the request will be made with the system (bearer or OAuth) token. For public routes you dont need to deifne any access token."),
                        saleTransactionHistoryId : z.string().uuid().describe("This id paremeter is used to select the required data object that will be deleted"),
...
return {
  ...
  parameters: DeleteSaleTransactionHistoryMcpController.getInputScheme(),
  controller:  async (mcpParams) => {
    mcpParams.headers =  headers;
  const deleteSaleTransactionHistoryMcpController =  new DeleteSaleTransactionHistoryMcpController(mcpParams);
  try {
    const result = await deleteSaleTransactionHistoryMcpController.processRequest();
    //return DeleteSaleTransactionHistoryMcpController.getOutputSchema().parse(result);  
    return {
      content: [
        {
          type: "text",
