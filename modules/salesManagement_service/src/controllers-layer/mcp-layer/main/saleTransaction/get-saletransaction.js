...
class GetSaleTransactionMcpController extends SalesManagementMcpController {
  constructor(params) {
    super("getSaleTransaction", "getsaletransaction", params);
    this.dataName = "saleTransaction";
    this.crudType = "get";
  }

  createApiManager() {
    return new GetSaleTransactionManager(this.request, "mcp");
  }
...
module.exports = (headers) => {
  return {
    ...
    description:
      "Get a sale transaction by ID. Allowed for seller, manager, or admin with appropriate permissions; store isolation enforced.",
    parameters: GetSaleTransactionMcpController.getInputScheme(),
    controller: async (mcpParams) => {
      mcpParams.headers = headers;
      const getSaleTransactionMcpController =
        new GetSaleTransactionMcpController(mcpParams);
      try {
        const result = await getSaleTransactionMcpController.processRequest();
        //return GetSaleTransactionMcpController.getOutputSchema().parse(result);
        return {
          content: [
            {
              type: "text",
