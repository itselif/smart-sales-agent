...
class CreateSaleTransactionMcpController extends SalesManagementMcpController {
  constructor(params) {
    super("createSaleTransaction", "createsaletransaction", params);
    this.dataName = "saleTransaction";
    this.crudType = "create";
  }

  createApiManager() {
    return new CreateSaleTransactionManager(this.request, "mcp");
  }
...
module.exports = (headers) => {
  return {
    ...
    description:
      "Creates a new sale transaction, sets sellerId from session, enforces storeId, and logs the creation event for audit.",
    parameters: CreateSaleTransactionMcpController.getInputScheme(),
    controller: async (mcpParams) => {
      mcpParams.headers = headers;
      const createSaleTransactionMcpController =
        new CreateSaleTransactionMcpController(mcpParams);
      try {
        const result =
          await createSaleTransactionMcpController.processRequest();
        //return CreateSaleTransactionMcpController.getOutputSchema().parse(result);
        return {
          content: [
            {
              type: "text",
