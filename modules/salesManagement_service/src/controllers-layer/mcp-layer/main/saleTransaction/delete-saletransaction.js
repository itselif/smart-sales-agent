...
class DeleteSaleTransactionMcpController extends SalesManagementMcpController {
  constructor(params) {
    super("deleteSaleTransaction", "deletesaletransaction", params);
    this.dataName = "saleTransaction";
    this.crudType = "delete";
  }

  createApiManager() {
    return new DeleteSaleTransactionManager(this.request, "mcp");
  }
...
module.exports = (headers) => {
  return {
    ...
    description:
      "Deletes a sale transaction (soft delete). Logs the deletion in saleTransactionHistory for audit compliance.",
    parameters: DeleteSaleTransactionMcpController.getInputScheme(),
    controller: async (mcpParams) => {
      mcpParams.headers = headers;
      const deleteSaleTransactionMcpController =
        new DeleteSaleTransactionMcpController(mcpParams);
      try {
        const result =
          await deleteSaleTransactionMcpController.processRequest();
        //return DeleteSaleTransactionMcpController.getOutputSchema().parse(result);
        return {
          content: [
            {
              type: "text",
