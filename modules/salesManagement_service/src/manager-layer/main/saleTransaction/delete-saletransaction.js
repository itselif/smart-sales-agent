...
const { dbDeleteSaletransaction } = require("dbLayer");

class DeleteSaleTransactionManager extends SaleTransactionManager {
  ...
  async doBusiness() {
    // Call DbFunction
    // make an awaited call to the dbDeleteSaletransaction function to delete the saletransaction and return the result to the controller
    const saletransaction = await dbDeleteSaletransaction(this);

    return saletransaction;
  }

  async raiseEvent() {
    SaletransactionDeletedPublisher.Publish(this.output, this.session).catch(
      (err) => {
  ...
  async executeAggregatedCruds() {
    this.addDeleteToHistory =
      await this.executeAggregatedCrudAddDeleteToHistory();
  }

  async executeAggregatedCrudAddDeleteToHistory() {
...
module.exports = DeleteSaleTransactionManager;
