...
const indexDataToElastic = async (data) => {
  const elasticIndexer = new ElasticIndexer("saleTransactionHistory");
  await elasticIndexer.indexData(data);
};

const updateSaleTransactionHistoryById = async (id, dataClause) => {
  try {
    if (!id && dataClause.id) {
      id = dataClause.id;
    ...
    const existingDoc = await SaleTransactionHistory.findOne({
    ...
    const [rowsCount, [dbDoc]] = await SaleTransactionHistory.update(
      dataClause,
      options,
    );
    if (!dbDoc) {
...
module.exports = updateSaleTransactionHistoryById;
