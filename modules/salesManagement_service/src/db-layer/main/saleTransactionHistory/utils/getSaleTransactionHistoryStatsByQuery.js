...
const getSaleTransactionHistoryStatsByQuery = async (query, stats) => {
  const promises = [];
  const statLabels = [];
  try {
    ...
    for (const stat of stats) {
      ...
      if (stat === "count") {
        promises.push(
          SaleTransactionHistory.count({ where: queryWithSoftDelete }),
        );
        statLabels.push("count");
      } else if (statParts.length == 2) {
        if (statParts[0] === "sum") {
          promises.push(
            SaleTransactionHistory.sum(statParts[1], {
          ...
          promises.push(
            SaleTransactionHistory.avg(statParts[1], {
              where: queryWithSoftDelete,
            }),
          );
          ...
          promises.push(
            SaleTransactionHistory.min(statParts[1], {
              where: queryWithSoftDelete,
            }),
          );
          ...
          promises.push(
            SaleTransactionHistory.max(statParts[1], {
              where: queryWithSoftDelete,
            }),
          );
    ...
    if (promises.length == 0) {
      return await SaleTransactionHistory.count({ where: queryWithSoftDelete });
...
module.exports = getSaleTransactionHistoryStatsByQuery;
