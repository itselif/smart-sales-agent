...
const indexDataToElastic = async (data) => {
  const elasticIndexer = new ElasticIndexer("saleTransaction");
  await elasticIndexer.indexData(data);
};

const updateSaleTransactionById = async (id, dataClause) => {
  try {
    if (!id && dataClause.id) {
      id = dataClause.id;
    ...
    const existingDoc = await SaleTransaction.findOne({
    ...
    const [rowsCount, [dbDoc]] = await SaleTransaction.update(
      dataClause,
      options,
    );
    if (!dbDoc) {
...
module.exports = updateSaleTransactionById;
