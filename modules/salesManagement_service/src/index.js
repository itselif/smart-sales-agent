...
console.log("Starting salesai-salesmanagement-service service...");
...
const start = async () => {
  process.title = "node-salesai1-salesmanagement-service";
  ...
  try {
    ...
    await setRedisData(
      "salesai1-salesmanagement-service:start",
      today.toUTCString(),
    );
    console.log(
      "Service start datetime is written to Redis key salesai1-salesmanagement-service:start",
      today.toUTCString(),
    );
  } catch (err) {
  ...
  console.log(
    "salesai-salesmanagement-service is listening HTTP/REST port " +
...
/* starting the server salesai-salesmanagement-service .... */
start();
