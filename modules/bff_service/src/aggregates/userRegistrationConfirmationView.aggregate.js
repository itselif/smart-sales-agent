...
const getAllUserRegistrationConfirmationView = async (filter = null) => {
  try {
...
const getUserRegistrationConfirmationView = async (id) => {
  try {
...
const userInfoAggregateDataFromIndex = async (source) => {
  const aggregation = await elasticClient.search({
    ...
    body: {
      ...
      _source: ["id", "email", "fullName", "username", "phone"],
    },
  });
...
const storeInfoAggregateDataFromIndex = async (source) => {
  const aggregation = await elasticClient.search({
    ...
    body: {
      ...
      _source: ["id", "name", "fullname", "city", "avatar", "active"],
    },
  });
