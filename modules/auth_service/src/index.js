...
console.log("Starting salesai-auth-service service...");
...
const SalesaiLoginSession = require("./project-session/salesai-login-session");

let processActive = false;
let expressServer = null;

const start = async () => {
  process.title = "node-salesai1-auth-service";
  ...
  try {
    ...
    await setRedisData("salesai1-auth-service:start", today.toUTCString());
    console.log(
      "Service start datetime is written to Redis key salesai1-auth-service:start",
      today.toUTCString(),
    );
  } catch (err) {
  ...
  console.log(
    "salesai-auth-service is listening HTTP/REST port " +
  ...
  // init login database
  const loginSession = new SalesaiLoginSession();
...
/* starting the server salesai-auth-service .... */
start();
