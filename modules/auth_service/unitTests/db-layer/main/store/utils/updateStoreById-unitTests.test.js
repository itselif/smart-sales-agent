...
describe("updateStoreById module", () => {
  let sandbox;
  let updateStoreById;
  let StoreStub, ElasticIndexerStub;

  const fakeId = "uuid-123";
  const fakeUpdatedData = { id: fakeId, name: "Updated Store" };
  ...
  beforeEach(() => {
    ...
    updateStoreById = proxyquire(
      "../../../../../src/db-layer/main/Store/utils/updateStoreById",
      {
  ...
  describe("updateStoreById", () => {
    it("should update record by direct ID", async () => {
      const result = await updateStoreById(fakeId, { name: "Updated" });
    ...
    it("should extract ID from dataClause if not explicitly provided", async () => {
      const result = await updateStoreById(undefined, {
        id: fakeId,
        name: "Updated",
      });
      expect(result).to.deep.equal(fakeUpdatedData);
    ...
    it("should support object-as-ID input", async () => {
      const result = await updateStoreById({ id: fakeId }, { name: "Updated" });
      expect(result).to.deep.equal(fakeUpdatedData);
    });
    ...
    it("should throw BadRequestError if no ID is provided", async () => {
      try {
        await updateStoreById(undefined, {});
    ...
    it("should throw NotFoundError if no existing record is found", async () => {
      ...
      try {
        await updateStoreById(fakeId, { name: "Updated" });
        throw new Error("Expected error");
      } catch (err) {
        expect(err.name).to.equal("HttpServerError");
    ...
    it("should throw HttpServerError if findOne fails", async () => {
      StoreStub.findOne.rejects(new Error("DB fail"));
      try {
        await updateStoreById(fakeId, { name: "Updated" });
        throw new Error("Expected error");
      } catch (err) {
