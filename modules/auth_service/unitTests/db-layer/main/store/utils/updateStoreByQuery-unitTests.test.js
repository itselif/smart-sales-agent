...
describe("updateStoreByQuery module", () => {
  let sandbox;
  let updateStoreByQuery;
  let StoreStub;
  ...
  beforeEach(() => {
    ...
    updateStoreByQuery = proxyquire(
      "../../../../../src/db-layer/main/Store/utils/updateStoreByQuery",
      {
  ...
  describe("updateStoreByQuery", () => {
    it("should update records matching query and return getData list", async () => {
      const result = await updateStoreByQuery(fakeDataClause, fakeQuery);
      ...
      const [calledDataClause, calledOptions] = StoreStub.update.firstCall.args;
    ...
    it("should throw BadRequestError if query is undefined", async () => {
      try {
        await updateStoreByQuery(fakeDataClause, undefined);
    ...
    it("should throw BadRequestError if query is not an object", async () => {
      try {
        await updateStoreByQuery(fakeDataClause, "not-an-object");
        throw new Error("Expected error");
      } catch (err) {
        expect(err.name).to.equal("HttpServerError");
        expect(err.details.name).to.equal("BadRequestError");
    ...
    it("should throw HttpServerError if model update fails", async () => {
      ...
      try {
        await updateStoreByQuery(fakeDataClause, fakeQuery);
        throw new Error("Expected error");
      } catch (err) {
        expect(err.name).to.equal("HttpServerError");
        expect(err.message).to.equal("errMsg_dbErrorWhenUpdatingStoreByQuery");
    ...
    it("should accept empty dataClause and still process", async () => {
      ...
      const result = await updateStoreByQuery({}, fakeQuery);
      expect(result).to.deep.equal([]);
    });
  });
});
