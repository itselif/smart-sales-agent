...
describe("createStore module", () => {
  let sandbox;
  let createStore;
  let StoreStub, ElasticIndexerStub, getNextCodenameStub, newUUIDStub;
  ...
  beforeEach(() => {
    ...
    getNextCodenameStub = sandbox.stub().resolves("generated-code");
    ...
    createStore = proxyquire(
      ...
      {
        "./getNextCodenameForStore.js": getNextCodenameStub,
  ...
  describe("createStore", () => {
    it("should create Store and index to elastic if valid data", async () => {
      ...
      sinon.assert.calledOnce(getNextCodenameStub);
    });
  ...
  describe("getNextCodename", () => {
    it("should call getNextCodename if codename is not provided", async () => {
      const input = getValidInput();
      delete input.codename;
      await createStore(input);
      sinon.assert.calledOnce(getNextCodenameStub);
    });

    it("should skip getNextCodename if codename is already set", async () => {
      const input = getValidInput({ codename: "preset-code" });
      await createStore(input);
      sinon.assert.notCalled(getNextCodenameStub);
    });

    it("should not call getNextCodename if name and codename are both missing", async () => {
      const input = getValidInput();
      delete input.codename;
      delete input["name"];
      await createStore(input);
